...
try:
    ...
    from prompts_hybrid import get_planning_prompt
...
def perceive(state):
    ...
    metrics_trend = analyze_metrics_trend(metrics_history, resolution, experiment_type)
...
def plan(state):
    ...
    2. Constructs prompt with workflow state and metrics trend
    3. Calls LLM to decide next action
    4. Falls back to mock if LLM unavailable
    """
...
def _mock_plan(state):
    """
    Fallback mock planner when LLM is unavailable.
    Uses workflow state to make reasonable decisions.
    """
    state = _log(state, "PLAN: Using mock planner (no LLM)")
    
    workflow_state = state.get("workflow_state", {})
    valid_programs = workflow_state.get("valid_programs", [])
    
    # If no workflow state, we can't make a good decision
    if not valid_programs:
        ...
        return {
            **state,
            "intent": {
                "program": None,
                "reasoning": "Mock: No workflow state available, stopping.",
                "files": {},
                "strategy": {},
                "stop": True,
    ...
    if program == "STOP":
        mock_intent = {
            "program": None,
            "reasoning": "Mock: No valid programs available, stopping.",
            "files": {},
            "strategy": {},
            "stop": True,
        ...
        mock_intent = {
            "program": program,
            "reasoning": "Mock: LLM unavailable, using first valid program from workflow state.",
...
def fallback(state):
    ...
    2. Trying alternative programs if first choice would duplicate
    3. For refine, preferring output files from previous cycles
    """
