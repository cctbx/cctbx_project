...
6. **V2 JSON API** - Clean client-server protocol with unified transport
7. **Local & Remote Agents** - Identical behavior whether running locally or via server
...
│                         Client Side                              │
├─────────────────────────────────────────────────────────────────┤
│  ai_agent.py → LocalAgent / RemoteAgent                         │
│                      │                                           │
│                      ▼                                           │
│  transport.prepare_request() → sanitize → JSON → encode         │
│                      │                                           │
...
│       │                                                          │
│       ▼                                                          │
│  transport.process_request() → decode → parse → execute graph   │
│       │                                                          │
│       ▼                                                          │
│  transport.prepare_response() → sanitize → JSON → encode        │
└─────────────────────────────────────────────────────────────────┘
```

## Configuration Files

The agent uses YAML configuration files in `knowledge/`:
...
| `api_schema.py` | V2 API request/response schema |
| `transport.yaml` | Transport encoding and sanitization settings |
...
improved_agent_v2/
├── agent/
│   ├── command_builder.py     # Unified command generation (4-stage pipeline)
...
│   ├── template_builder.py    # Legacy command builder (delegates to CommandBuilder)
│   ├── sanity_checker.py      # Validation and red flag detection
│   ├── best_files_tracker.py  # File quality scoring and tracking
...
│   ├── session.py             # Session management and summary generation
│   ├── api_client.py          # V2 API request/response building
│   └── transport.py           # Unified transport encoding/decoding
├── analysis/
│   ├── __init__.py            # Analysis module exports
│   └── agent_session_analyzer.py  # LLM-based session assessment
├── knowledge/
...
│   ├── transport.yaml         # Transport configuration
│   ├── yaml_loader.py         # YAML access functions
│   └── prompts_hybrid.py      # LLM prompt templates (incl. session assessment)
├── tests/
│   ├── run_all_tests.py       # Master test runner (13 test suites)
│   ├── test_api_schema.py     # API schema tests
│   ├── test_best_files_tracker.py
│   ├── test_command_builder.py # CommandBuilder tests
│   ├── test_dry_run.py
│   ├── test_file_categorization.py # File classification tests
│   ├── test_integration.py
│   ├── test_metrics_analyzer.py
│   ├── test_sanity_checker.py
│   ├── test_session_summary.py # Session summary generation tests
│   ├── test_state_serialization.py # State round-trip tests
│   ├── test_transport.py      # Transport tests (71 tests)
│   ├── test_workflow_state.py
│   ├── test_yaml_config.py
│   └── scenarios/             # Dry run test scenarios
├── docs/
│   ├── README.md              # Documentation index
│   ├── ARCHITECTURE.md        # System architecture (incl. session summary)
│   ├── API_DOCUMENTATION.md   # V2 API reference
│   └── VALIDATION.md          # Sanity checking and red flags
├── local_agent.py             # Local execution agent
├── remote_agent.py            # Remote server agent
├── ai_agent.py                # Main AI agent class
├── run_ai_agent.py            # Server-side entry point
├── yaml_tools.py              # YAML validation & inspection
...
python tests/test_state_serialization.py
```
...
|----------|-------------|
| `AGENT_SUMMARY.md` | This file - overview and quick start |
| `docs/ARCHITECTURE.md` | System architecture, CommandBuilder, data flow |
| `docs/API_DOCUMENTATION.md` | V2 API request/response reference |
| `docs/VALIDATION.md` | Sanity checking, red flags, validation gates |
...
```json
{
  ...
  "log_content": "Previous program output...",
  "session_state": {
  ...
  "history": [...],
  "user_advice": "Focus on ligand fitting",
  "settings": {
    "provider": "google",
...
The transport layer ensures reliable client-server communication:
...
# Prepare request (sanitize + JSON + encode)
...
- Removes ZZxxZZ REST encoding markers from logs
- Truncates long quoted strings (e.g., pdb70_text data dumps)
- Replaces tabs with spaces
- Removes control characters
- Configurable via `knowledge/transport.yaml`
