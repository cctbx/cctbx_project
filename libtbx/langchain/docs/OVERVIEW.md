...
| Version | Key Changes |
|---------|-------------|
| v121 | **`phenix.ai_analysis` standard mode: single-call replacement.** The report path no longer summarises, chunks or retrieves: it reads the log, resolves the program, and makes **one long-context LLM call on the whole log**. Six measured defects are removed by construction — the analysis step read a ~1 KB summary (0.7–8.4% of the log retained on 6 of 7 logs); `_custom_log_chunker` dropped everything after `Citations` when *"Files are in the directory"* appeared (7 of 20 corpus logs, 59% of a 304 KB log); the program was misidentified on the server path (6 wrong of 20); a fabricated name went into the prompt (`"Log file for phenix.xtriage_1"`, 19 of 20); retrieval keyed on a query that was byte-identical for every log; and the summariser could emit **more** than it was given (382,721 chars from 156,951, reproducibly; 2,026,595 from 312,011). New modules `analysis/program_identity.py` (caller → banner → file name → refuse, 20/20 zero wrong), `analysis/analysis_request.py` (log embedded once, byte for byte, SHA-256 verified), `analysis/report_verifier.py` (shadow mode), `analysis/orientation_strip.py` (deterministic `summary_file`, replacing a byte-identical copy of the report). **No mode needs the RAG database any more** — `standard` joined `_LLM_ONLY_MODES` and three database gates were removed, which had been refusing runs for `anthropic`. Measured: better on 3 logs, tied on 5, worse on none against the old path; run-to-run flip rate 2% against 8%. Verified on a 659 KB log across google, openai and anthropic, all citing to 99.8% of the log. 73 tests in 5 suites. **Not established: behaviour above ~660 KB.** |
...
the Phenix server.  **As of v121 no analysis mode needs the RAG database**,
so local execution is never impossible for want of one: `standard` stopped
retrieving when it became a single long-context call on the whole log.  All
five modes (standard, directive_extraction, advice_preprocessing,
failure_diagnosis, agent_session) run local under the flag with no
`docs_db_<provider>` present — verified with `anthropic`, which has no
database at all, on a 659 KB log.  Behaviour is otherwise identical to
server mode; only the locus of execution changes.
...
both regardless.  **As of v121 `phenix.ai_analysis` needs no embeddings
at all** — `standard` mode makes one long-context call and does not
retrieve, so a chat-only provider runs it with no database.  The
remaining consumers of embeddings are the database-build tools
(`rebuild_ai_database` / `update_ai_database`) and `run_query_docs.py`.
For `anthropic`, embeddings still transparently delegate to OpenAI or
Google when a key is available; `anthropic` remains intentionally not
offered by the database-build tools, since it cannot produce
embeddings.
...
│   ├── analyzer.py             # Log analysis (single long-context call, v121)
│   ├── program_identity.py     # Which program produced this log (v121)
│   ├── analysis_request.py     # Builds the single call; hashes the log (v121)
│   ├── report_verifier.py      # Checks report figures against the log (v121)
│   ├── orientation_strip.py    # Deterministic summary_file content (v121)
│   ├── log_info.py             # High-level log info extraction
│   ├── state_extractor.py      # Project state extraction from logs
│   ├── summarizer.py           # Map-reduce summarization (NOT on the v121
│   │                           #   report path; unreachable from run())
