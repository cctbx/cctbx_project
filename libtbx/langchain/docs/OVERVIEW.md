...
| Version | Key Changes |
|---------|-------------|
| v115.09 | **Tutorial Routing Fixes**: (1) Cryo-EM `past_analysis` gate: added `map_sharpening_done`, `map_symmetry_done`, `has_optimized_full_map` — unblocks bgal_denmod, apoferritin_denmod, ion_channel_denmod after map_sharpening. (2) `.sca-only` data detection in `perceive()` with helpful abort message for p9-xtriage. (3) Validation-only routing: `wants_validation_only` directive extracted via LLM prompt + rules-based fallback → validation shortcut in `_detect_xray_step` → `validate_existing` plan template (model_vs_data + molprobity). (4) MR-SAD routing: `force_mr` flag when `use_mr_sad` + model not categorized as search_model → phaser offered without file reclassification; MR-SAD guard updated. Intent detection in directive extractor layer (not engine string-matching). 5 files modified, ~80 lines. |
