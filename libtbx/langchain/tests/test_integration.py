...
def test_cryoem_workflow_detection():
    ...
    assert state["workflow_state"]["valid_programs"] == ["phenix.mtriage"]
...
def test_file_path_validation():
    ...
    state = create_initial_state(
        available_files=["data.mtz", correct_path],
        history=[{"program": "phenix.xtriage", "result": "SUCCESS"}]
    )
    state = perceive(state)
    ...
    assert state["validation_error"] is None, "Expected validation to pass with matching basename"
