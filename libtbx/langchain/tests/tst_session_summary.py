...
def test_extract_summary_data():
  ...
  # Check workflow path
  assert (
    "AlphaFold" in data["workflow_path"]
    or "molecular replacement"
    in data["workflow_path"].lower())
...
def test_markdown_generation():
  ...
  # Either "Phenix AI Run:" or "Phenix AI Tutorial:"
  assert "## Phenix AI" in markdown
...
def test_quality_assessments():
  ...
  # Stricter at high res
  assert session._assess_r_free(
    0.22, 1.0) == "Acceptable"
...
"""
  ...
  try:
    metrics = session._extract_metrics_from_result(
      result_text, "phenix.xtriage")
...
def test_history_preserves_anomalous_across_restart():
  ...
  assert analysis.get("has_anomalous") == True, (
    f"Expected has_anomalous=True in history,"
    f" got {analysis.get('has_anomalous')}")

  assert analysis.get("anomalous_measurability") == 0.15, (
    f"Expected anomalous_measurability=0.15"
    f" in history, got "
    f"{analysis.get('anomalous_measurability')}")
...
def test_failure_diagnosis_path_in_summary():
  ...
  assert "## Failure Diagnosis" in markdown, (
    "Results markdown must contain"
    " '## Failure Diagnosis' section"
    " when diagnosis fired")
  ...
  result2 = session2.generate_agent_session_summary(
    include_llm_assessment=False)
