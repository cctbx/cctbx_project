...
# If dependencies are missing, all tests are skipped with a warning.
_SKIP_REASON = None
try:
...
def _skip_if_deps_missing(cls):
    """Class decorator: skip all tests in a class if dependencies are missing."""
    if _SKIP_REASON:
...
class TestAnalysisModule(unittest.TestCase):
    ...
    def test_get_log_map_prompt(self):
        ...
        # Should mention extraction and log analysis
        self.assertTrue(
            "extraction" in prompt.template.lower() or
            "extract" in prompt.template.lower(),
            "Log map prompt should mention extraction"
        )
    ...
    def test_get_log_combine_prompt(self):
        ...
        # Should mention crystallography/summary
        self.assertTrue(
            "crystallograph" in prompt.template.lower() or
            "summary" in prompt.template.lower(),
            "Log combine prompt should mention crystallography or summary"
        )
...
class TestKnowledgeModule(unittest.TestCase):
    ...
    def test_get_docs_query_prompt(self):
        ...
        self.assertIsInstance(prompt, PromptTemplate)
        # Should mention Phenix documentation
        self.assertTrue(
            "phenix" in prompt.template.lower() or
            "Phenix" in prompt.template,
            "Docs query prompt should mention Phenix"
        )
...
def run_all_tests():
    ...
    if failures > 0:
        raise AssertionError("%d test(s) failed" % failures)
