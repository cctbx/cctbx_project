...
def test_xray_has_prediction():
    """Test X-ray after predict_and_build - needs process_predicted_model."""
    ...
    assert state["state"] == "xray_has_prediction"
    assert state["valid_programs"] == ["phenix.process_predicted_model"]
...
def test_xray_model_processed():
    ...
    assert state["state"] == "xray_model_processed"
    assert state["valid_programs"] == ["phenix.phaser"]
...
def test_xray_has_model():
    ...
    assert state["state"] == "xray_has_model"
    assert state["valid_programs"] == ["phenix.refine"]
...
def test_autobuild_priority_over_ligandfit():
    """Test that autobuild is prioritized over ligandfit when model is poor."""
    print("Test: autobuild_priority_over_ligandfit")
    
    # With poor R-free and ligand available, autobuild should come before ligandfit
    ...
    # Poor model (R-free 0.40) - autobuild should be higher priority than ligandfit
    # New threshold is 0.35, so 0.40 is clearly above it
    state = detect_workflow_state(history, files, analysis={"r_free": 0.40})
    assert "phenix.autobuild" in state["valid_programs"], "autobuild should be available with R-free 0.40"
    assert "phenix.ligandfit" in state["valid_programs"]
    # Autobuild should come before ligandfit
    autobuild_idx = state["valid_programs"].index("phenix.autobuild")
    ligandfit_idx = state["valid_programs"].index("phenix.ligandfit")
    assert autobuild_idx < ligandfit_idx, "Autobuild should be higher priority than ligandfit for poor model"
    
    # Good model (R-free 0.30) - ligandfit should be available, autobuild should NOT be
    # New threshold is 0.35, so 0.30 is below it
    state2 = detect_workflow_state(history, files, analysis={"r_free": 0.30})
    assert "phenix.ligandfit" in state2["valid_programs"]
    # autobuild should NOT be available for good model
    assert "phenix.autobuild" not in state2["valid_programs"], "autobuild not needed with R-free 0.30"
...
def test_xray_refined_with_ligand():
    ...
    assert state["state"] == "xray_refined"
    assert "phenix.ligandfit" in state["valid_programs"]
    assert state["valid_programs"][0] == "phenix.ligandfit"  # Priority
...
def test_xray_has_ligand():
    ...
    assert state["state"] == "xray_has_ligand"
    assert state["valid_programs"] == ["phenix.pdbtools"]
...
def test_xray_combined():
    ...
    assert state["state"] == "xray_combined"
    assert "phenix.refine" in state["valid_programs"]
    assert "STOP" in state["valid_programs"]
...
def test_cryoem_initial():
    ...
    assert state["state"] == "cryoem_initial"
...
def test_cryoem_analyzed_automated():
    ...
    assert state["state"] == "cryoem_analyzed"
    assert "phenix.predict_and_build" in state["valid_programs"]
    assert state["automation_path"] == "automated"
...
def test_cryoem_has_model_automated():
    ...
    assert state["state"] == "cryoem_has_model"
...
def test_cryoem_refined():
    ...
    assert state["state"] == "cryoem_refined"
...
def test_cryoem_has_prediction_stepwise():
    """Test cryo-EM after predict with stop_after_predict=True."""
    ...
    assert state["state"] == "cryoem_has_prediction"
    assert state["valid_programs"] == ["phenix.process_predicted_model"]
...
def test_cryoem_model_processed_stepwise():
    """Test cryo-EM after process_predicted_model."""
    ...
    assert state["state"] == "cryoem_model_processed"
    assert state["valid_programs"] == ["phenix.phaser"]
...
def test_cryoem_has_phases_stepwise():
    """Test cryo-EM after phaser - needs autobuild."""
    ...
    assert state["state"] == "cryoem_has_phases"
    assert state["valid_programs"] == ["phenix.autobuild"]
...
def test_ligandfit_with_cryoem():
    """Test ligandfit availability for cryo-EM refined state."""
