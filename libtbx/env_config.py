...
@set LIBTBX_OPATH=%PATH%''', file=f)
  ...
  def write_Makefile(self):
    if self.build_options.use_conda \
        and os.name == "posix" \
        and self.build_options.compiler == "conda" \
        and os.path.exists(os.path.join(get_conda_prefix(), "bin", "activate")) \
        and os.path.exists("/bin/bash"):
      conda_path = os.path.relpath(get_conda_prefix(), abs(self.build_path))
      lsj = 'source %s/bin/activate && ./bin/libtbx.scons -j "`./bin/libtbx.show_number_of_processors`"' % conda_path
