...
class six_dof_body(object):
  ...
  def __init__(O):
    sites = matrix.col_list([
      ...
      (0.405, 3.954, 5.917),
      (0.779, 5.262, 5.227)])
    O.A = joint_lib.six_dof_alignment(sites=sites)
...
class spherical_body(object):
  ...
  def __init__(O):
    sites = matrix.col_list([
      (0.04, -0.16, 0.19),
      (0.10, -0.15, 0.18)])
    O.A = joint_lib.spherical_alignment(sites=sites)
    O.I = utils.spatial_inertia_from_sites(sites=sites, alignment_T=O.A.T0b)
...
def exercise_system_model():
  model = featherstone.system_model(
    bodies=[six_dof_body(), spherical_body(), revolute_body()])
  assert approx_equal(model.e_kin(), 5.04553665235)
  qdd = matrix.col_list([
