...
namespace scitbx { namespace af { namespace boost_python {
  ...
  flex<uint64_t>::type*
  __u64from_std_string(const_ref<std::string> const& s)
  {
    shared<uint64_t> result(reserve(s.size()));
    for(std::size_t i=0;i<s.size();i++) {
      ...
      uint64_t value = 0;
      try {
        value = boost::lexical_cast<uint64_t>(s[i]);
      }
    ...
    return new flex<uint64_t>::type(result, result.size());
  }

  shared<bool>
  as_bool(const_ref<uint64_t> const& self, bool strict=true)
  {
  ...
  as_long(
    af::const_ref<uint64_t, af::flex_grid<> > const& O)
  {
  ...
  /* For allowed syntax for the optional format_string argument see:
     ...
     .. in here using dedicated formatting statements for size
   */
  af::shared<std::string>
  as_string(
    af::const_ref<uint64_t, af::flex_grid<> > const& O,
    std::string format_string="%+" PRIu64)
  {
    std::size_t n = O.accessor().size_1d();
  ...
  af::shared<uint64_t> bitwise_not(
      af::const_ref<uint64_t> const &self) {
    af::shared<uint64_t> result(self.size());
    for (std::size_t i = 0; i < self.size(); ++i) {
      result[i] = ~self[i];
    }
    return result;
  ...
  af::shared<uint64_t> bitwise_or_single(
      af::const_ref<uint64_t> const &self,
      uint64_t other) {
    af::shared<uint64_t> result(self.size());
    for (std::size_t i = 0; i < self.size(); ++i) {
  ...
  af::shared<uint64_t> bitwise_or_array(
      af::const_ref<uint64_t> const &self,
      af::const_ref<uint64_t> const &other) {
  ...
  af::shared<uint64_t> bitwise_and_single(
      af::const_ref<uint64_t> const &self,
      uint64_t other) {
    af::shared<uint64_t> result(self.size());
    for (std::size_t i = 0; i < self.size(); ++i) {
  ...
  af::shared<uint64_t> bitwise_and_array(
      af::const_ref<uint64_t> const &self,
      af::const_ref<uint64_t> const &other) {
    SCITBX_ASSERT(self.size() == other.size());
    af::shared<uint64_t> result(self.size());
  ...
  af::shared<uint64_t> bitwise_xor_single(
      af::const_ref<uint64_t> const &self,
      uint64_t other) {
    af::shared<uint64_t> result(self.size());
    for (std::size_t i = 0; i < self.size(); ++i) {
  ...
  af::shared<uint64_t> bitwise_xor_array(
      af::const_ref<uint64_t> const &self,
      af::const_ref<uint64_t> const &other) {
    SCITBX_ASSERT(self.size() == other.size());
    af::shared<uint64_t> result(self.size());
  ...
  void wrap_flex_uint64_t()
  {
    using namespace boost::python;
    using boost::python::arg;
    flex_wrapper<uint64_t>::integer("uint64", scope())
      .def_pickle(flex_pickle_single_buffered<uint64_t>())
      .def("__init__", make_constructor(
        __u64from_std_string, default_call_policies()))
      .def("__init__", make_constructor(
        flex_uint64_from_numpy_array, default_call_policies()))
      .def("copy_to_byte_str", copy_to_byte_str<versa<uint64_t, flex_grid<> > >)
      .def("slice_to_byte_str",
        slice_to_byte_str<versa<uint64_t, flex_grid<> > >)
      ...
      .def("as_rgb_scale_string", as_rgb_scale_string, (
        ...
        arg("rgb_scales_high"),
        arg("saturation")))
      .def("counts", counts<uint64_t, std::map<long, long> >::unlimited)
      .def("counts", counts<uint64_t, std::map<long, long> >::limited, (
        arg("max_keys")))
      .def("matrix_is_symmetric",
        (bool(*)(
          const_ref<uint64_t, c_grid<2> > const&))
            matrix::is_symmetric)
      .def("matrix_copy_block",
        (versa<uint64_t, c_grid<2> >(*)(
          const_ref<uint64_t, c_grid<2> > const&,
      ...
      .def("matrix_transpose_in_place",
        (void(*)(versa<uint64_t, flex_grid<> >&)) matrix_transpose_in_place)
      .def("matrix_rot90",
        (versa<uint64_t, c_grid<2> >(*)(
           const_ref<uint64_t, c_grid<2> > const&, int)) matrix_rot90)
      .def("matrix_swap_rows_in_place",
        (void(*)(
          ref<uint64_t, c_grid<2> > const&, unsigned, unsigned))
        ...
        (void(*)(
          ...
          unsigned, unsigned))
            matrix::paste_block_in_place, (
              ...
              arg("i_row"),
              arg("i_column")))
      .def("as_numpy_array", flex_uint64_as_numpy_array, (
        arg("optional")=false))
    ...
    ;
    def(
      "uint64_from_byte_str",
      shared_from_byte_str<uint64_t>,
      (arg("byte_str")));
  }
