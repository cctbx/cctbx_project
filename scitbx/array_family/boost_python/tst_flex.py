...
def exercise_mean_and_variance():
  ...
  assert approx_equal(mv.variance(), 1)
  assert approx_equal(mv.standard_deviation(), 1)
  w = flex.double((1,1,1))
  ...
  assert approx_equal(mv.variance(),
    (6./(36-14))*((1*(1-13/6.)**2)+(3*(2-13/6.)**2)+(2*(3-13/6.)**2)))
  assert approx_equal(mv.standard_deviation(), math.sqrt(17/22.))
