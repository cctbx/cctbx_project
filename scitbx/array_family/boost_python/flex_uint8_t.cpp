...
namespace scitbx { namespace af { namespace boost_python {
  ...
  flex<uint8_t>::type*
  __u8from_std_string(const_ref<std::string> const& s)
  {
    shared<uint8_t> result(reserve(s.size()));
    for(std::size_t i=0;i<s.size();i++) {
      ...
      uint8_t value = 0;
      try {
        value = boost::lexical_cast<uint8_t>(s[i]);
      }
    ...
    return new flex<uint8_t>::type(result, result.size());
  }

  shared<bool>
  as_bool(const_ref<uint8_t> const& self, bool strict=true)
  {
  ...
  as_long(
    af::const_ref<uint8_t, af::flex_grid<> > const& O)
  {
  ...
  as_string(
    af::const_ref<uint8_t, af::flex_grid<> > const& O,
    std::string format_string="%+" PRIu8)
  {
    std::size_t n = O.accessor().size_1d();
  ...
  af::shared<uint8_t> bitwise_not(
      af::const_ref<uint8_t> const &self) {
    af::shared<uint8_t> result(self.size());
    for (std::size_t i = 0; i < self.size(); ++i) {
      result[i] = ~self[i];
    }
    return result;
  ...
  af::shared<uint8_t> bitwise_or_single(
      af::const_ref<uint8_t> const &self,
      uint8_t other) {
    af::shared<uint8_t> result(self.size());
    for (std::size_t i = 0; i < self.size(); ++i) {
  ...
  af::shared<uint8_t> bitwise_or_array(
      af::const_ref<uint8_t> const &self,
      af::const_ref<uint8_t> const &other) {
  ...
  af::shared<uint8_t> bitwise_and_single(
      af::const_ref<uint8_t> const &self,
      uint8_t other) {
    af::shared<uint8_t> result(self.size());
    for (std::size_t i = 0; i < self.size(); ++i) {
  ...
  af::shared<uint8_t> bitwise_and_array(
      af::const_ref<uint8_t> const &self,
      af::const_ref<uint8_t> const &other) {
    SCITBX_ASSERT(self.size() == other.size());
    af::shared<uint8_t> result(self.size());
  ...
  af::shared<uint8_t> bitwise_xor_single(
      af::const_ref<uint8_t> const &self,
      uint8_t other) {
    af::shared<uint8_t> result(self.size());
    for (std::size_t i = 0; i < self.size(); ++i) {
  ...
  af::shared<uint8_t> bitwise_xor_array(
      af::const_ref<uint8_t> const &self,
      af::const_ref<uint8_t> const &other) {
    SCITBX_ASSERT(self.size() == other.size());
    af::shared<uint8_t> result(self.size());
  ...
  void wrap_flex_uint8_t()
  {
    ...
    flex_wrapper<uint8_t>::integer("uint8", scope())
      .def_pickle(flex_pickle_single_buffered<uint8_t>())
      .def("__init__", make_constructor(
        __u8from_std_string, default_call_policies()))
      .def("__init__", make_constructor(
        flex_uint8_from_numpy_array, default_call_policies()))
      .def("copy_to_byte_str", copy_to_byte_str<versa<uint8_t, flex_grid<> > >)
      .def("slice_to_byte_str",
        slice_to_byte_str<versa<uint8_t, flex_grid<> > >)
      ...
      .def("as_rgb_scale_string", as_rgb_scale_string, (
        ...
        arg("rgb_scales_high"),
        arg("saturation")))
      .def("counts", counts<uint8_t, std::map<long, long> >::unlimited)
      .def("counts", counts<uint8_t, std::map<long, long> >::limited, (
        arg("max_keys")))
      .def("matrix_is_symmetric",
        (bool(*)(
          const_ref<uint8_t, c_grid<2> > const&))
            matrix::is_symmetric)
      .def("matrix_copy_block",
        (versa<uint8_t, c_grid<2> >(*)(
          const_ref<uint8_t, c_grid<2> > const&,
      ...
      .def("matrix_transpose_in_place",
        (void(*)(versa<uint8_t, flex_grid<> >&)) matrix_transpose_in_place)
      .def("matrix_rot90",
        (versa<uint8_t, c_grid<2> >(*)(
           const_ref<uint8_t, c_grid<2> > const&, int)) matrix_rot90)
      .def("matrix_swap_rows_in_place",
        (void(*)(
          ref<uint8_t, c_grid<2> > const&, unsigned, unsigned))
        ...
        (void(*)(
          ...
          unsigned, unsigned))
            matrix::paste_block_in_place, (
              ...
              arg("i_row"),
              arg("i_column")))
      .def("as_numpy_array", flex_uint8_as_numpy_array, (
        arg("optional")=false))
