...
def exercise_eigensystem():
  ...
  for n in range(1,10):
    ...
    if (n == 3):
      ...
      assert approx_equal(list(s.values()), list(ss.values()))
      assert approx_equal(s.vectors(), ss.vectors())
    v.sort()
    v.reverse()
    assert approx_equal(list(s.values()), v)
    if (n > 1):
  ...
  for n_power in range(5,max_n_power+1):
    ...
    for i,use_fortran in enumerate([False, True]):
      ...
      if (info == 99):
        ...
        assert approx_equal(list(reversed(list(es.values()))), wla[i])
    if (show_tab_header):
      print("      time [s]           eigenvalues")
      print(" n    es   fem   for     min    max")
      show_tab_header = False
    tab.extend([list(es.values())[-1], list(es.values())[0]])
