...
namespace scitbx { namespace fn {""", file=f)
  ...
  for op, ftor_name in list(operator_functor_info.unary_functors.items()):
    generate_unary(f, ftor_name, op + "x")
  for op, ftor_name in list(operator_functor_info.binary_functors.items()):
    generate_binary(f, ftor_name, "x " + op + " y")
  for op, ftor_name in list(operator_functor_info.in_place_binary_functors.items()):
