...
class FullCBFWriter(object):
  ...
  def __init__(self, filename = None, imageset = None):
    ...
    assert [filename, imageset].count(None) == 1, "Supply either filename or imageset"
  ...
  def get_cbf_handle(self, index=None, header_only = False, detector_only = False):
    ...
    def recursive_setup_basis_dict(key, parent_name = "", panel_id = 0):
      ...
      if not node.is_panel():
        for c, child in enumerate(node):
          panel_id = recursive_setup_basis_dict(tuple(list(key) + [c]), dname, panel_id)
      return panel_id
    ...
    cbf.add_row([diffrn_id,detector_id,"General dxtbx detector",".",str(len(detector_axes_names))])
    ...
    for i, panelname in enumerate(panelnames):
      cbf.add_row(["FRAME1","ELE_"+panelname,"ARRAY_"+panelname,"%d"%(i+1)])

    if not detector_only:
      add_frame_specific_cbf_tables(cbf, beam.get_wavelength(), "unknown",
    ...
    cbf.add_row(("%s_Z         translation detector .        0  0  1 . . . %s"%(dname,eqc)).split())       ; axis_names.append("%s_Z"%dname)
    cbf.add_row(("%s_Y         translation detector %s_Z     0  1  0 . . . %s"%(dname,dname,eqc)).split()) ; axis_names.append("%s_Y"%dname)
    cbf.add_row(("%s_X         translation detector %s_Y     1  0  0 . . . %s"%(dname,dname,eqc)).split()) ; axis_names.append("%s_X"%dname)
    ...
    for key in sorted(metro):
      ...
      if node.is_panel():
        ...
        cbf.add_row(["AXIS_"+ aname + "_S", "translation", "detector", basis.axis_name,
                     slow[0], slow[1], slow[2], "0", "0", "0", basis.equipment_component])
        cbf.add_row(["AXIS_"+ aname + "_F", "translation", "detector", "AXIS_"+aname +"_S",
                     fast[0], fast[1], fast[2], "0", "0", "0", basis.equipment_component])
    ...
    for panelkey, panelname in zip(panelkeys, panelnames):
      ...
      cbf.add_row(["ARRAY_"+panelname,"1","%d"%fast_dim,"1","increasing","AXIS_"+aname+"_F"])
      cbf.add_row(["ARRAY_"+panelname,"2","%d"%slow_dim,"2","increasing","AXIS_"+aname+"_S"])
    ...
    for panelkey in panelkeys:
      ...
      cbf.add_row(["AXIS_"+aname+"_F","AXIS_"+aname+"_F","0.0",str(pixel_size[0])])
      cbf.add_row(["AXIS_"+aname+"_S","AXIS_"+aname+"_S","0.0",str(pixel_size[1])])
