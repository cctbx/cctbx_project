...
def build_run(a_out, n_scatt, n_refl, build_cmd, check_max_a_b):
  ...
  if (a_out is None):
    ...
    if (buffers.format_errors_if_any() is not None):
      return None
  else:
...
def write_build_run(
      a_out, n_scatt, n_refl, real, lang, build_cmd, replace_cos, replace_exp):
  if (a_out is None):
...
def run_combinations(
      ...
      real_list):
  for lang,compiler,build_opts in compiler_build_opts_list:
    ...
    for real in real_list:
      ...
      for replace_cos in [False, True]:
        ...
        for replace_exp in [False, True]:
          ...
          utimes.append(utime)
          sys.stdout.flush()
    all_utimes.append((utimes, build_cmd))
...
def usage():
  raise Usage(
    "fable.python sf_times.py unit_test|quick|production|a_out_archive.pickle")
...
def run(args):
  ...
  run_combinations(
    ...
    compiler_build_opts_list=[
      ("for", "ifort", "-O"),
      ("for", "gfortran", gcc_static+"-O -ffast-math"),
      ("for", "gfortran", gcc_static+"-O -ffast-math -march=native"),
      ("for", "g77", gcc_static+"-O -ffast-math"),
      ("cpp", "icpc", "-static -O"),
      ("cpp", "g++", gcc_static+"-O -ffast-math"),
      ("cpp", "g++", gcc_static+"-O -ffast-math -march=native")],
