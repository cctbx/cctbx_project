...
def make_proxies(model=None,
  ...
  if planes is not None:
    plane_source_info = DummySourceInfo(n_expected_atoms=4)
    for t in planes.itertuples():
      i_seqs = flex.size_t([int(getattr(t,f"i_seq_{i}")) for i in range(len(planes.columns)) if hasattr(t,f"i_seq_{i}") and getattr(t,f"i_seq_{i}") is not pd.NA])
      weights = flex.double([int(getattr(t,f"weight_{i}")) for i in range(len(planes.columns)) if hasattr(t,f"weight_{i}") and getattr(t,f"weight_{i}") is not pd.NA])
...
class RestraintsBuilder:
  ...
  def build_grm_from_component_mol(self,component_mol):
    geometry_proxy_registries = make_proxies(model=component_mol.model,
                                           bonds=component_mol.bonds,
                                           angles=component_mol.angles,
                                           torsions=component_mol.torsions,
                                           planes=component_mol.planes,
                                           )
    prep_dict = prep_for_grm(component_mol.sites,
                                    component_mol.model,
                                    geometry_proxy_registries)


    grm = build_grm(model=component_mol.model,
                    geometry_proxy_registries=geometry_proxy_registries,
                    nonbonded_types=component_mol.sites.nonbonded_types,
                    nonbonded_charges=component_mol.sites.nonbonded_charges,
                    prep=prep_dict
                    )
    return grm
