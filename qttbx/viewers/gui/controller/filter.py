...
class FilterByOrigin(FilterObj):
  def __init__(self,origin_label):
    # Origin label, set by parse_geo_file
    self.origin_label = origin_label
    name = "Origin "+origin_label.capitalize()
    super().__init__(name=name,mmcif_prefix="origin")

  def filter_df(self,df):
    cols = self._get_filter_cols(df)
    return df[(df[cols]==self.origin_label).all(axis=1)]
...
class TableFilterController(Controller):
  ...
  def _init_filters(self):
    ...
    # Add origin filters
    if "origin" in self.table.df.columns:
      for origin_name in list(self.table.df.origin.unique()):
        filter_obj = FilterByOrigin(origin_name)
        self.other_filters.append(filter_obj)
