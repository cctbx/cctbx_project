...
from ...state.geometry import Geometry, GeoFileData
from ...state.ref import GeometryRef, GeoFileRef
...
class GeoFileListController(ScrollableListController):
  ...
  def showFileDialog(self):
    ...
    if fname[0]:
      ...
      # add to state
      data = GeoFileData(filepath=filepath)
      ref = GeoFileRef(data=data,show=True)
      self.state.add_ref(ref)



  def update(self):
    references_geofiles = [value for key,value in self.state.references.items() if isinstance(value,GeoFileRef)]
    for ref in references_geofiles:
