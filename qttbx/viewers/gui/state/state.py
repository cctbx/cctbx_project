...
from ...core.parameters import params as parameters
...
class StateSignals(QObject):
  ...
  # Geometry signals
  geometry_present = Signal(bool) # Signal that actions requiring geometry are possible
...
class State:
  ...
  def __init__(self, data_manager,params=None,log=None):
    self._log = log
    self._params = params
    ...
    self._phenix_state = PhenixState(references={},has_synced=False) # Rename molstar state
    self.debug = True
  ...
  def add_ref(self,ref,emit=True):
    ...
    if isinstance(ref,ModelRef):
      ...
      self.signals.geometry_present.emit(True)

      # check for edits present
      edits = self.params.geometry_restraints.edits
      self.add_edits_from_phil(edits)
  ...
  def add_edits_from_phil(self,edits):
    # Given an edits scope extract, add edit objects 

    # Create objects
    bonds = [BondEdit.from_phil_extract(param) for param in edits.bond]
    angles = [AngleEdit.from_phil_extract(param) for param in edits.angle]
    dihedrals = [DihedralEdit.from_phil_extract(param) for param in edits.dihedral]
    # TODO: More than dihedrals

    edit_objects = bonds + angles + dihedrals
    if len(edit_objects)>0:
      # Add labels
      sites = self.mol.sites
      for obj in edit_objects:
        obj.add_labels_from_sites(sites)
      
      # Add to state
      for edit_obj in edit_objects:
        self.add_ref_from_edit(edit_obj)
      
      self.signals.tab_change.emit("Edits") # show  tab

  def add_ref_from_edit(self,edit_obj):
    # Add an edit object to state with corresponding ref
    ref_class_dict = {
      BondEdit:BondEditsRef,
      AngleEdit:AngleEditsRef,
      DihedralEdit:DihedralEditsRef,
      ChiralEdit:ChiralEditsRef,
      PlaneEdit:PlaneEditsRef
    }
    assert edit_obj.__class__ in ref_class_dict
    RefClass = ref_class_dict[edit_obj.__class__]
    edit_ref = None
    for ref_id,ref in self.references.items():
      # Check if ref already exists for this object
      if isinstance(ref,RefClass):
        if ref.geometry_ref == self.active_model_ref.geometry_ref:
          edit_ref = ref
          edit_ref.data.rows.append(edit_obj)
          
    # Add ref to state, if a novel ref, create it
    if edit_ref is None:
      objframe = ObjectFrame.from_rows([edit_obj])
      edit_ref = RefClass(data=objframe,geometry_ref=self.state.active_model_ref.geometry_ref)
    self.add_ref(edit_ref)
