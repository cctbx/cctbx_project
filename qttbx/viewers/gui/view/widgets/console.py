...
class JupyterTabWidget(GUITab):
  def __init__(self,parent=None):
    ...
    Access the program template instance with 'program'.

    For example:
      {'program':<40} : {'cctbx ProgramTemplate instance':<40}
      {'program.view':<40} : {'the Qt main window':<40}
      {'program.controller.molstar':<40} : {'python interface for Mol*':<40}
      {'program.state.data_manager':<40} : {'the cctbx data manager':<40}
    """
