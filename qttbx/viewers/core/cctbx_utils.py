...
def extract_to_dict(obj, visited=None):
  # Phil scope extract to dict
  if visited is None:
    visited = set()
  ...
  for key in dir(obj):
    # Ignore attributes prefixed with '_'
    if key.startswith('_'):
      continue
    ...
    if isinstance(value, scope_extract):
      result[key] = extract_to_dict(value, visited)
    elif isinstance(value, list):
      # Process each item in the list
      result[key] = [extract_to_dict(item, visited) if isinstance(item, scope_extract) else item for item in value]
    else:
      result[key] = value
