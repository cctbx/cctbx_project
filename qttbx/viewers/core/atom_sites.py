...
AtomSites subclasses pandas DataFrame. 
  ...
  4. A companion object to Selection class, it acts as a common data structure 
      ...
      issue that that data structures are not comprehensively convertable. This 
...
class AtomSites(pd.DataFrame):
  ...
  def _verify_selection_with_cctbx(self,pandas_string=None,phenix_string=None):
    ...
    try:
      ...
      self.log("Failed to query a selection on the dataframe using string:")
      self.log(pandas_string)
      self.log()
      self.log("Comparison Phenix string:")
      self.log(phenix_string)
      raise
    ...
    if not np.all(np.isclose(xyz_sel,xyz_sel_cctbx)):
      self.log("Failed to match cartesian coordinates between selection and cctbx selection")
      self.log("CCTBX string: ",phenix_string)
      self.log("Pandas string: ",pandas_string)
  ...
  def select_from_pandas_string(self,pandas_str,verify_phenix_str=None):
    ...
    if verify_phenix_str:
      ...
      assert passed
      assert isinstance(sites_sel,self.__class__)
      self.log("PASSED CCTBX CHECK")
    else:
      try:
        sites_sel = self.query(pandas_str)
      except:
        self.log("Failed to select from pandas string:")
        self.log(pandas_str)
        raise
    return sites_sel
  ...
  def to_labels_compositional(self):
    ...
    for record in records:
      label = ""
      for attr,j in self.params.compositional_labels_justify_map.items():
        value = record[attr]
  ...
  def _pandas_query_to_sites(self,pandas_query,do_copy=True):
    try:
      ...
      self.log("Failed to query from pandas query:")
      self.log(pandas_query)
  ...
  #   except:
  #     self.log("Failed to select from pandas query:")
  #     self.log(pandas_query)
  ...
  #   """
  #   assert isinstance(sites_sel,self.__class__), f"Provide an atom sites object, not: {type(sites_sel), {self.log(sites_sel)}}"
...
def compare_hierarchies(h_left,h_right):
  ...
  self.log("starting hierarchical compare")
        ...
        for ag_left,ag_right in zip(rg_left.atom_groups(),rg_right.atom_groups()):
          ...
          for atom_left,atom_right in zip(ag_left.atoms(),ag_right.atoms()):
            ...
            if atom_left.name != atom_right.name:
               self.log("Atom names differ in whitespace.")
  ...
  if not h_right.is_similar_hierarchy(h_left):
     self.log("Hierarchies are similar but do not pass is_similar_hierarchy()")
  return True
...
def compare_sites(sites1, sites2,skip_columns=[]):
    ...
    # Display the results
    #self.log("Columns in df1 missing in df2:", missing_in_df2)
    #self.log("Columns in df2 missing in df1:", missing_in_df1)
