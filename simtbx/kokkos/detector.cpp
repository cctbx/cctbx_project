...
namespace simtbx { namespace Kokkos {
  ...
  packed_metrology::packed_metrology(dxtbx::model::Detector const & arg_detector,
    ...
    for (std::size_t panel_id = 0; panel_id < arg_detector.size(); panel_id++) {
      ...
      typedef scitbx::vec3<double> scitbx_vec3;
      auto get_kokkos_vector3 = [](scitbx_vec3 src) { return vector3(src[0], src[1], src[2]); };

      // DETECTOR properties
      // typically: 1 0 0
      vector3 fdet_vector = get_kokkos_vector3( arg_detector[panel_id].get_fast_axis() );
      fdet_vector.normalize();

      // typically: 0 -1 0
      vector3 sdet_vector = get_kokkos_vector3( arg_detector[panel_id].get_slow_axis() );
      sdet_vector.normalize();

      // set orthogonal vector to the detector pixel array
      vector3 odet_vector = fdet_vector.cross(sdet_vector);
      odet_vector.normalize();

      // dxtbx origin is location of outer corner of the first pixel
      vector3 pix0_vector = get_kokkos_vector3( arg_detector[panel_id].get_origin() )/1000.0;

      // what is the point of closest approach between sample and detector?
      CUDAREAL close_distance = pix0_vector.dot(odet_vector);
      if (close_distance < 0) {
        bool verbose = false;
  ...
  packed_metrology::packed_metrology(const simtbx::nanoBragg::nanoBragg& nB) {
    auto get_kokkos_vec3 = [](auto& src) { return vector3(src[1], src[2], src[3]); };
  ...
  packed_metrology::show() const {
    for (std::size_t idx_p = 0; idx_p < Xbeam.size(); idx_p++){
      ...
      printf(" Panel %3ld sdet %9.6f %9.6f %9.6f fdet %9.6f %9.6f %9.6f\n",
             idx_p,sdet[idx_p].x,sdet[idx_p].y,sdet[idx_p].z,
                   fdet[idx_p].x,fdet[idx_p].y,fdet[idx_p].z
      );
      printf(" Panel %3ld odet %9.6f %9.6f %9.6f pix0 %9.6f %9.6f %9.6f\n",
             idx_p,odet[idx_p].x,odet[idx_p].y,odet[idx_p].z,
                   pix0[idx_p].x,pix0[idx_p].y,pix0[idx_p].z
      );
