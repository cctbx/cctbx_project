#ifndef SIMTBX_KOKKOS_STRUCTURE_FACTORS_H
#define SIMTBX_KOKKOS_STRUCTURE_FACTORS_H
...
struct kokkos_energy_channels {
  ...
  inline
  void structure_factors_replace_GPU_direct(
    int const& ichannel, indices pythony_indices, af::shared<double> pythony_amplitudes){
    double F_cell;
    // FIXME clearly this is duplicated code and could be refactored
    /* load the structure factors */
    SCITBX_ASSERT(pythony_indices.size()>0);
    if(pythony_indices.size()) {
        if(verbose) printf(" noticed pythony_indices.size() = %ld\n",pythony_indices.size());
    ...
    structure_factors_replace_KOKKOS_detail(ichannel, linear_amplitudes);
  }

  void structure_factors_replace_KOKKOS_detail(int const&, af::shared<double>);
