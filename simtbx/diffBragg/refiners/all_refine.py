...
class RefineAll(RefineRot):
    ...
    def _setup(self):
        ...
        self.x[-4] = np.log(self.S.crystal.Ncells_abc[0])
    ...
    def _update_ncells(self):
        self.D.set_value(self._ncells_id, np.exp(self.x[-4]))
    ...
    def compute_functional_and_gradients(self):
        ...
        for i_spot in range(self.n_spots):
            ...
            if self.refine_ncells:
                ...
                g[-4] += (S2*G2*self.ncells_deriv*one_minus_k_over_Lambda*np.exp(self.x[-4])).sum()
    ...
    def print_step(self, message, target):
        ...
        print("%s: residual=%3.8g, ncells=%f, detdist=%8.4g, gain=%8.4g, scale=%8.4g"
              % (message, target, np.exp(self.x[-4]), self.x[-3], self.x[-2]**2, self.x[-1]**2))
