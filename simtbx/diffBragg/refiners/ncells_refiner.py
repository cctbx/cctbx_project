...
class RefineNcells(RefineRot):

    def __init__(self, init_gain=1, init_scale=1, *args, **kwargs):
        """
        :param ucell_manager:
        :param rotXYZ_refine:
        :param args:
        ...
        self._ncells_id = 9  #TODO make this less clunky, this is the derivative manager ID for detector distance
        self.best_image = np.zeros_like(self.img)
        self.refine_ncells = True

    def _setup(self):
        # total number of refinement parameters
        n_bg = 3 * self.n_spots
        n_spotscale = 2
        n_ncells = 1# number of Ncells abc parameters (1 for cubic domain model)
        self.n = n_bg + n_ncells + n_spotscale
        self.x = flex.double(self.n)

        # populate the x-array with initial values
        self._move_abc_init_to_x()
        self.x[-3] = np.log(self.S.crystal.Ncells_abc[0])
    ...
    def _update_ncells(self):
        self.D.set_value(self._ncells_id, np.exp(self.x[-3]))
    ...
    def compute_functional_and_gradients(self):
        ...
        for i_spot in range(self.n_spots):
            ...
            if self.refine_ncells:
                g[-3] += (S2*G2*self.ncells_deriv*one_minus_k_over_Lambda*np.exp(self.x[-3])).sum()
    ...
    def print_step(self, message, target):
        print("%s: residual=%2.7g, ncells=%f, gain=%2.7g, scale=%2.7g"
              % (message, target, np.exp(self.x[-3]), self.x[-2], self.x[-1]))
