    ...
    for (int i_pix=0; i_pix < Npix_to_model; i_pix++){
        ...
        for (int i_step=0; i_step < db_steps.Nsteps; i_step++){
            ...
            if (db_flags.use_diffuse){
                ...
                Eigen::Matrix3d anisoG(db_cryst.anisoG.data);
                Eigen::Matrix3d anisoU(db_cryst.anisoU.data);
                ...
                for (int hh=0; hh <1; hh++){
                    for (int kk=0; kk <1; kk++){
                        for (int ll=0; ll <1; ll++){
                            ...
                            double exparg = 4*M_PI*M_PI*Q0.dot(anisoU*Q0);
                            ...
                            */
                            ...
                            if (db_flags.refine_diffuse){
                                 ...
                                 for (int i_sig = 0;i_sig<3; i_sig++){
                                     Eigen::Matrix3d dU_dsigma(db_cryst.dU_dsigma[i_sig].data);
                                   double dexparg = 4*M_PI*M_PI*Q0.dot(dU_dsigma*Q0);
                                   /*                              if (exparg  >= .5) // only valid up to a point
                                     dexparg = 0;
                                   */
        ...
        if (!db_flags.track_Fhkl){
            ...
            if (!db_flags.nopolar){
                ...
                if(db_beam.kahn_factor != 0.0){
                    ...
                    Eigen::Vector3d B_in = Eigen::Vector3d(db_beam.polarization_axis.data).cross(incident);
