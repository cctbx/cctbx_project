...
void diffBraggKOKKOS::diffBragg_sum_over_steps_kokkos(
    ...
    // static vector_uint_t m_panels_fasts_slows = vector_uint_t("m_panels_fasts_slows", 0);
...
//  support dynamic allocation for different numbers of sources
    ...
    if(m_device_is_allocated){
        ...
        if (db_cryst.UMATS_RXYZ_prime.size()>0)
            ...
            // gpuErr(cudaMallocManaged((void **)&m_cu_UMATS_RXYZ_prime, db_cryst.UMATS_RXYZ_prime.size()*sizeof(MAT3)));
        if (db_cryst.UMATS_RXYZ_dbl_prime.size()>0)
            resize(m_UMATS_RXYZ_dbl_prime, db_cryst.UMATS_RXYZ_dbl_prime.size());
            // gpuErr(cudaMallocManaged((void **)&m_cu_UMATS_RXYZ_dbl_prime, db_cryst.UMATS_RXYZ_dbl_prime.size()*sizeof(MAT3)));

        resize(m_dB_Mats, db_cryst.dB_Mats.size());
        resize(m_dB2_Mats, db_cryst.dB2_Mats.size());
        // gpuErr(cudaMallocManaged((void **)&m_cu_dB_Mats, db_cryst.dB_Mats.size()*sizeof(MAT3)));
        // gpuErr(cudaMallocManaged((void **)&m_cu_dB2_Mats, db_cryst.dB2_Mats.size()*sizeof(MAT3)));

        resize(m_RotMats, db_cryst.RotMats.size());
        resize(m_dRotMats, db_cryst.dRotMats.size());
        resize(m_d2RotMats, db_cryst.d2RotMats.size());
        // gpuErr(cudaMallocManaged((void **)&m_cu_RotMats, db_cryst.RotMats.size()*sizeof(MAT3)));
        // gpuErr(cudaMallocManaged((void **)&m_cu_dRotMats, db_cryst.dRotMats.size()*sizeof(MAT3)));
        // gpuErr(cudaMallocManaged((void **)&m_cu_d2RotMats, db_cryst.d2RotMats.size()*sizeof(MAT3)));
        ...
        // gpuErr(cudaMallocManaged(&m_cu_fdet_vectors, db_det.fdet_vectors.size()*sizeof(CUDAREAL)));
        // gpuErr(cudaMallocManaged(&m_cu_sdet_vectors, db_det.fdet_vectors.size()*sizeof(CUDAREAL)));
        // gpuErr(cudaMallocManaged(&m_cu_odet_vectors, db_det.fdet_vectors.size()*sizeof(CUDAREAL)));
        // gpuErr(cudaMallocManaged(&m_cu_pix0_vectors, db_det.fdet_vectors.size()*sizeof(CUDAREAL)));
        // gpuErr(cudaMallocManaged(&m_cu_close_distances, db_det.close_distances.size()*sizeof(CUDAREAL)));

        if (db_cryst.fpfdp.size() > 0){
            resize(m_fpfdp, db_cryst.fpfdp.size());
            resize(m_atom_data, db_cryst.atom_data.size());
            // gpuErr(cudaMallocManaged(&m_cu_fpfdp, db_cryst.fpfdp.size()*sizeof(CUDAREAL)));
            // gpuErr(cudaMallocManaged(&m_cu_atom_data, db_cryst.atom_data.size()*sizeof(CUDAREAL)));
        }
        if(db_cryst.fpfdp_derivs.size() > 0)
            resize(m_fpfdp_derivs, db_cryst.fpfdp_derivs.size());
            // gpuErr(cudaMallocManaged(&m_cu_fpfdp_derivs, db_cryst.fpfdp_derivs.size()*sizeof(CUDAREAL)));
        ...
        // gpuErr(cudaMallocManaged(&m_cu_Fhkl, db_cryst.FhklLinear.size()*sizeof(CUDAREAL)));
        if (db_flags.complex_miller)
            resize(m_Fhkl2, db_cryst.FhklLinear.size());
            // gpuErr(cudaMallocManaged(&m_cu_Fhkl2, db_cryst.FhklLinear.size()*sizeof(CUDAREAL)));

        resize(m_dF_vecs, db_det.dF_vecs.size());
        resize(m_dS_vecs, db_det.dF_vecs.size());
        // gpuErr(cudaMallocManaged((void **)&m_cu_dF_vecs, db_det.dF_vecs.size()*sizeof(VEC3)));
        // gpuErr(cudaMallocManaged((void **)&m_cu_dS_vecs, db_det.dF_vecs.size()*sizeof(VEC3)));

        //gettimeofday(&t3, 0));
        resize(m_floatimage, db_cu_flags.Npix_to_allocate);
        // gpuErr(cudaMallocManaged(&m_cu_floatimage, db_cu_flags.Npix_to_allocate*sizeof(CUDAREAL) ));
        if (db_flags.wavelength_img){
            resize(m_wavelenimage, db_cu_flags.Npix_to_allocate);
            // gpuErr(cudaMallocManaged(&m_cu_wavelenimage, db_cu_flags.Npix_to_allocate*sizeof(CUDAREAL) ));
        }
        if (db_flags.refine_diffuse){
            ...
            // gpuErr(cudaMallocManaged(&m_cu_d_diffuse_gamma_images, db_cu_flags.Npix_to_allocate*3*sizeof(CUDAREAL)));
            // gpuErr(cudaMallocManaged(&m_cu_d_diffuse_sigma_images, db_cu_flags.Npix_to_allocate*3*sizeof(CUDAREAL)));
        }
        if (db_flags.refine_fcell){
            ...
            // gpuErr(cudaMallocManaged(&m_cu_d_fcell_images, db_cu_flags.Npix_to_allocate*1*sizeof(CUDAREAL)));
            // gpuErr(cudaMallocManaged(&m_cu_d2_fcell_images, db_cu_flags.Npix_to_allocate*1*sizeof(CUDAREAL)));
        }
        if (db_flags.refine_eta){
            ...
            // gpuErr(cudaMallocManaged(&m_cu_d_eta_images, db_cu_flags.Npix_to_allocate*3*sizeof(CUDAREAL)));
            // gpuErr(cudaMallocManaged(&m_cu_d2_eta_images, db_cu_flags.Npix_to_allocate*3*sizeof(CUDAREAL)));
        }
        if (std::count(db_flags.refine_Umat.begin(), db_flags.refine_Umat.end(), true) > 0){
            ...
            // gpuErr(cudaMallocManaged(&m_cu_d_Umat_images, db_cu_flags.Npix_to_allocate*3*sizeof(CUDAREAL) ));
            // gpuErr(cudaMallocManaged(&m_cu_d2_Umat_images, db_cu_flags.Npix_to_allocate*3*sizeof(CUDAREAL) ));
        }
        if (std::count(db_flags.refine_Ncells.begin(), db_flags.refine_Ncells.end(), true) > 0 || db_flags.refine_Ncells_def){
            ...
            // gpuErr(cudaMallocManaged(&m_cu_d_Ncells_images, db_cu_flags.Npix_to_allocate*6*sizeof(CUDAREAL)));
            // gpuErr(cudaMallocManaged(&m_cu_d2_Ncells_images, db_cu_flags.Npix_to_allocate*6*sizeof(CUDAREAL)));
        }
        if (std::count(db_flags.refine_panel_rot.begin(), db_flags.refine_panel_rot.end(), true) > 0)
            ...
            // gpuErr(cudaMallocManaged(&m_cu_d_panel_rot_images, db_cu_flags.Npix_to_allocate*3*sizeof(CUDAREAL)));
        if (std::count(db_flags.refine_panel_origin.begin(), db_flags.refine_panel_origin.end(), true) > 0)
            ...
            // gpuErr(cudaMallocManaged(&m_cu_d_panel_orig_images, db_cu_flags.Npix_to_allocate*3*sizeof(CUDAREAL)));
        if (std::count(db_flags.refine_lambda.begin(), db_flags.refine_lambda.end(), true) > 0)
            ...
            // gpuErr(cudaMallocManaged(&m_cu_d_lambda_images, db_cu_flags.Npix_to_allocate*2*sizeof(CUDAREAL)));
        if (std::count(db_flags.refine_Bmat.begin(), db_flags.refine_Bmat.end(), true) > 0){
            ...
            // gpuErr(cudaMallocManaged(&m_cu_d_Bmat_images, db_cu_flags.Npix_to_allocate*6*sizeof(CUDAREAL)));
            // gpuErr(cudaMallocManaged(&m_cu_d2_Bmat_images, db_cu_flags.Npix_to_allocate*6*sizeof(CUDAREAL)));
        }
        if (db_flags.refine_fp_fdp)
            resize(m_d_fp_fdp_images, db_cu_flags.Npix_to_allocate*2);
            // gpuErr(cudaMallocManaged(&m_cu_d_fp_fdp_images, db_cu_flags.Npix_to_allocate*2*sizeof(CUDAREAL)));
        if(db_cryst.nominal_hkl.size() >0)
            resize(m_nominal_hkl, db_cu_flags.Npix_to_allocate*3);
            // gpuErr(cudaMallocManaged(&m_cu_nominal_hkl, db_cu_flags.Npix_to_allocate*3*sizeof(int)));
        ...
        // gpuErr(cudaMallocManaged(&m_cu_panels_fasts_slows, db_cu_flags.Npix_to_allocate*3*sizeof(panels_fasts_slows[0])));
...
//  BEGIN sources
    if (db_cu_flags.update_sources || ALLOC || FORCE_COPY){
        ...
        Kokkos::parallel_for("normalize incident vector", source_count, KOKKOS_LAMBDA (const int& i) {
...
//  END panels fasts slows
    ...
    gpu_sum_over_steps<<<numblocks, blocksize>>>(
        Npix_to_model, m_cu_panels_fasts_slows,
        m_cu_floatimage,
        m_cu_wavelenimage,
        m_cu_d_Umat_images, m_cu_d2_Umat_images,
        m_cu_d_Bmat_images, m_cu_d2_Bmat_images,
        m_cu_d_Ncells_images, m_cu_d2_Ncells_images,
        m_cu_d_fcell_images, m_cu_d2_fcell_images,
        m_cu_d_eta_images, m_cu_d2_eta_images,
        m_cu_d_lambda_images, m_cu_d2_lambda_images,
        m_cu_d_panel_rot_images, m_cu_d2_panel_rot_images,
        m_cu_d_panel_orig_images, m_cu_d2_panel_orig_images,
        m_cu_d_fp_fdp_images,
        ...
        db_det.detector_thickstep, db_det.detector_thick, m_cu_close_distances, db_det.detector_attnlen,
        ...
        m_cu_dS_vecs,
        m_cu_UMATS_RXYZ,
        m_cu_UMATS_RXYZ_prime,
        m_cu_UMATS_RXYZ_dbl_prime,
        m_cu_RotMats,
        m_cu_dRotMats,
        m_cu_d2RotMats,
        m_cu_UMATS,
        ...
        m_cu_Fhkl, m_cu_Fhkl2,
        m_cu_refine_Bmat, m_cu_refine_Ncells, db_flags.refine_Ncells_def, m_cu_refine_panel_origin, m_cu_refine_panel_rot,
        db_flags.refine_fcell, m_cu_refine_lambda, db_flags.refine_eta, m_cu_refine_Umat,
        m_cu_fdet_vectors, m_cu_sdet_vectors,
        m_cu_odet_vectors, m_cu_pix0_vectors,
        ...
        m_cu_fpfdp,  m_cu_fpfdp_derivs, m_cu_atom_data, num_atoms,
        db_flags.refine_fp_fdp, m_cu_nominal_hkl, use_nominal_hkl, db_cryst.anisoU, db_cryst.anisoG, db_flags.use_diffuse,
        m_cu_d_diffuse_gamma_images, m_cu_d_diffuse_sigma_images,
