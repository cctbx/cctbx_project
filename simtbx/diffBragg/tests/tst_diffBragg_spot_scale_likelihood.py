...
for i_shot in range(N_SHOTS):
   ...
   #     spot_roi, tilt_abc = utils.process_simdata(SPOTS2, img/omega_kahn, thresh=20, plot=args.plot, shoebox_sz=20)
    ...
    spot_refls = utils.refls_from_sims(np.array([spotimg]), SIM.detector, SIM.beam.nanoBragg_constructor_beam,
                                       thresh=20)
    out = utils.get_roi_background_and_selection_flags(spot_refls, np.array([img]), shoebox_sz=20,
