...
namespace cctbx { namespace sgtbx {
  ...
  class SpaceGroup {
    private:
      ...
      TrVec m_InvT;
      bool  m_isTidy;
      af::tiny<RTMx, nMaxReprRotMx> m_SMx;
    ...
    public:
      ...
      /*! See also: expandSMx()
       */
      template <typename RTMxArrayType>
      void expandSMxArray(const RTMxArrayType& SMxArray) {
        for(std::size_t i=0;i<SMxArray.size();i++) {
          expandSMx(SMxArray[i]);
        }
      }
       ...
       */
      bool isCompatibleMetricalMatrix(const af::double9& G,
       ...
       */
      void CheckMetricalMatrix(const af::double9& G,
                               double tolerance = 1.e-4) const;
      //! Check if a unit cell is compatible with the symmetry operations.
  ...
  expand_to_p1(
    ...
    const MillerIndexArrayType& in,
    MillerIndexArrayType& out,
    bool friedel_flag = false) // XXX counter-intuitive semantics
  {
    ...
    bool friedel_flag = false) // XXX counter-intuitive semantics
  {
    ...
    for(std::size_t i_in = 0; i_in < h_in.size(); i_in++) {
      SymEquivMillerIndices semi = SgOps.getEquivMillerIndices(h_in[i_in]);
      for (int i_list = 0; i_list < semi.N(); i_list++) {
        for (int i_mate = 0; i_mate < semi.fMates(friedel_flag); i_mate++) {
          h_out.push_back(semi(i_mate, i_list));
          if (ampl_in.size()) {
          ...
          if (phase_in.size()) {
            if (i_mate == 0) {
              if (phase_degrees) {
                phase_out.push_back(
                  semi[i_list].phase_eq_deg(phase_in[i_in]));
              }
              else {
                phase_out.push_back(
                  semi[i_list].phase_eq_rad(phase_in[i_in]));
              }
            }
            else {
              if (phase_degrees) {
