...
namespace cctbx { namespace maps {
  ...
  void
  peak_search_unit_cell(const DataVersaType& data,
                        TagsArrayType& tags,
                        int level)
  {
    typedef typename DataVersaType::value_type data_value_type;
    typedef typename TagsArrayType::value_type tags_value_type;
  ...
  af::shared<
    indexed_value<
      typename DataVersaType::accessor_type::index_type,
      typename DataVersaType::value_type,
      std::greater<typename DataVersaType::value_type> >
  >
  collect_peaks(const DataVersaType& data,
                const TagsArrayType& tags,
                const typename DataVersaType::value_type& cutoff,
  ...
  struct peak_histogram
  {
    ...
    template <typename DataVersaType,
              typename TagsArrayType>
    peak_histogram(const DataVersaType& data,
                   const TagsArrayType& tags,
    ...
    af::shared<count_type> slots;
  };


  template <typename ValueType,
            typename IndexType = af::tiny<long, 3> >
  class peak_list
    : public af::shared<
        indexed_value<IndexType, ValueType, std::greater<ValueType> > >
  {
    public:
      ...
      typedef
        ...
        entry_type;
      typedef af::shared<entry_type> base_type;

      peak_list() {}

      template <typename DataVersaType,
                typename TagsArrayType>
      peak_list(const DataVersaType& data,
                TagsArrayType& tags,
                ...
                std::size_t max_peaks = 0)
      {
        ...
        *(static_cast<base_type*>(this)) = collect_peaks(
