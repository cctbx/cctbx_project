    ...
    public:
        ...
        double xtol)
      {
        ...
        if (iflag_ == 0) { // Initialize.
          ...
          gnorm_ = std::sqrt(ddot(n, g, 0, 1, g, 0, 1));
          stp1= 1/gnorm_;
          ftol= 0.0001;
          stp_ = double(0);
        ...
        for (;;) {
          ...
          if (!mcsrch_instance.run(params, n, x, f, g, w, ispt + point * n,
                stp_, ftol, xtol, maxfev, info, nfev, diag)) {
            log_.push_back(
          ...
          double xnorm = std::max(
            double(1), std::sqrt(ddot(n, x, 0, 1, x, 0, 1)));
          if (gnorm_ / xnorm <= eps) {
            iflag_ = 0;
            break;
          }
      ...
      // XXX n Number of free parameters.
      // XXX m Number of corrections kept.
