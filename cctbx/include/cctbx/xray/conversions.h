...
namespace cctbx { namespace xray {
  ...
  struct f_sq_as_f
  {
    f_sq_as_f(FloatType const& f_sq,
  ...
  {
    array_f_sq_as_f(af::const_ref<FloatType> const& f_sq,
                    ...
                    FloatType const& tolerance=1.e-6)
    {
      ...
      for(std::size_t i=0;i<f_sq.size();i++) {
        f_sq_as_f<FloatType> r(f_sq[i], sigma_f_sq[i], tolerance);
        f.push_back(r.f);
