...
class box_sharpening_info(object):
...
class sharpening_info(object):
  ...
  def show_summary(self,verbose=False,out=sys.stdout):
    ...
    if self.sharpening_method=="b_iso":
      ...
      for d_min,sc in zip(
        ...
        print("Dmin: %7.2f  Scale: %7.2f" %(d_min,sc), file=out)
    ...
    if self.sharpening_method in ["b_iso_to_d_cut"] and \
      self.k_sharpen and self.resolution:
        print("Transition from sharpening", file=out)
        print("   to not sharpening (k_sharpen):%7.2f " %(self.k_sharpen), file=out)
  ...
  def sharpen_and_score_map(self,map_data=None,set_b_iso=False,out=sys.stdout):
    ...
    print("B-iso before sharpening:", end=' ', file=out)
    ...
    print("B-iso after sharpening:", end=' ', file=out)
...
def get_map_object(file_name=None,out=sys.stdout):
  ...
  print("\nCrystal symmetry used: ", file=out)
...
def get_ncs_from_map(map_data=None,
  ...
  if optimize_center is None:
    if ncs_center is None and (not use_center_of_map_as_center):
      ...
      print("Setting optimize_center=True as no ncs_center is supplied", file=out)
    else:
  ...
  print("Center of NCS (A): (%7.3f, %7.3f, %7.3f) " %(
    tuple(ncs_center)), file=out)

  print("\nFinding %s NCS" %(ncs_type), file=out)
  ...
  for ncs_obj,ncs_type in zip(ncs_list,ncs_type_list):
    ...
    if score is None:
      print("ncs_type:",ncs_type," no score",ncs_obj.max_operators(), file=out)
    else:
  ...
  for score,cc_avg,ncs_obj,ncs_type in results_list:
    print(" %6.2f  %5.2f    %2d          %s" %(
       score,cc_avg,ncs_obj.max_operators(), ncs_type.strip(),), file=out)
  ...
  if optimize_center:
    ...
    print("New center: (%7.3f, %7.3f, %7.3f)" %(tuple(ncs_center)), file=out)

  if cc_avg < min_ncs_cc:
    print("No suitable symmetry found with center of map as center...\n", file=out)
    return None
...
def get_ncs_list(ncs_type,
  ...
  if ncs_center and tuple(ncs_center) != (0,0,0,):
    print("Offsetting NCS center by (%.2f, %.2f, %.2f) A " %(tuple(ncs_center)), file=out)
    new_list=[]
...
def get_params(args,map_data=None,crystal_symmetry=None,out=sys.stdout):
  ...
  if  params.segmentation.density_select:
    ...
    if params.map_modification.density_select_in_auto_sharpen:
      ...
      print("Turning off density_select_in_auto_sharpen as "+\
         "it will be done here", file=out)
...
def get_ncs(params,tracking_data=None,ncs_object=None,out=sys.stdout):
  ...
  if file_name: print("Reading ncs from %s" %(file_name), file=out)
  is_helical_symmetry=None
...
def score_threshold(b_vs_region=None,threshold=None,
   ...
   if not b_vs_region.b_iso in list(b_vs_region.b_vs_region_dict.keys()):
...
def create_rna_dna(cns_dna_rna_residue_names):
  dd={}
  for key in list(cns_dna_rna_residue_names.keys()):
...
def top_key(dd):
  if not dd:
    return None,None
  elif len(list(dd.keys()))==1:
    return list(dd.keys())[0],dd[list(dd.keys())[0]]
  else:
    best_key=None
    best_n=None
...
def run_get_duplicates_and_ncs(
  ...
  missing=[]
  for i in range(1,max_regions_to_consider+1):
    if not i in list(region_centroid_dict.keys()):
...
def get_duplicates_and_ncs(
  ...
  for id in list(region_scattered_points_dict.keys()):
      ...
      for xyz_cart in region_scattered_points_dict[id]:
        ...
        for i0 in range(len(ncs_group.translations_orth())):
          ...
          if value==id:
            ...
            if not value in list(equiv_dict[id].keys()):
              equiv_dict[id][value]=0
              equiv_dict_ncs_copy_dict[id][value]={}
            equiv_dict[id][value]+=1
            if not n in list(equiv_dict_ncs_copy_dict[id][value].keys()):
...
def get_ncs_equivalents(
  ...
  for id in region_list:
    ...
    for id1 in key_list:
      ...
      if n<min_coverage*region_scattered_points_dict[id].size():
        ...
        if not id in list(equiv_dict_ncs_copy.keys()):equiv_dict_ncs_copy[id]={}
  ...
  # Skipping this below
  print("\nSets of NCS-related regions", file=out)
  keys=list(equiv_dict_ncs_copy.keys())
  keys.sort()
  used=[]
  for id in keys:
    #if id in used: continue
    others=list(equiv_dict_ncs_copy[id].keys())
    used+=others
    print("%d: " %(id), end=' ', file=out)
    for id1 in others:
      key,n=top_key(equiv_dict_ncs_copy_dict[id][id1])
      print("%d:%d" %(id1,n), end=' ', file=out)
    print(file=out)
  print(file=out)
...
def group_ncs_equivalents(params,
  ...
  for id in region_list:
    ...
    for id1 in list(equiv_dict_ncs_copy.get(id,{}).keys()):
      ncs_copy=equiv_dict_ncs_copy[id][id1]
      if not ncs_copy in list(equiv_group.keys()): equiv_group[ncs_copy]=[]
    ...
    if tracking_data.input_ncs_info.is_helical_symmetry:
      ...
      if len(missing_ncs_copies)==0:
        ...
        complete=False
    if complete and \
        (not str(equiv_group_as_list) in list(ncs_equiv_groups_as_dict.keys()) or
  ...
  for ncs_group in ncs_group_list:
    for group_list in ncs_group:
      for id1 in group_list:
        if not id1 in list(shared_group_dict.keys()): shared_group_dict[id1]=[]
...
def get_inter_region_dist_dict(ncs_group_obj=None,
  ...
  for i in range(len(selected_regions)):
    ...
    if not id in list(dd.keys()): dd[id]={}
    ...
    for j in range(i+1,len(selected_regions)):
      ...
      dd[id][id1]=dist
      if not id1 in list(dd.keys()): dd[id1]={}
      dd[id1][id]=dist
  return dd
...
def get_closest_neighbor_rms(ncs_group_obj=None,selected_regions=None,
  ...
  if verbose:
    ...
    keys.sort()
    for key in keys:
      for key2 in list(inter_region_dist_dict[key].keys()):
...
def write_region_maps(params,
  ...
  for id in ncs_group_obj.selected_regions:
    ...
    if remainder_ncs_group_obj:
      for remainder_id in list(remainder_ncs_group_obj.remainder_id_dict.keys()):
...
def apply_origin_shift(origin_shift=None,
  ...
  if shifted_ncs_file:
      ...
      if tracking_data.input_ncs_info.has_updated_operators():
        print("NOTE: these may include additional operators added to fill the cell", file=out)
...
def get_overall_mask(
  ...
  mean_in_map=mm.mean
  min_in_map=mm.min
  print("Highest value in map is %7.2f. Mean is %7.2f .  Lowest is %7.2f " %(
    max_in_map,
    mean_in_map,
    min_in_map), file=out)
...
def put_bounds_in_range(
  ...
  if buffer:
     print("Buffer of %s added" %(buffer), file=out)
...
def select_box_map_data(si=None,
  ...
  if pdb_inp or si.density_select_in_auto_sharpen:  # use map_box
    ...
    if si.density_select_in_auto_sharpen:
      ...
      print("Using density_select in map_box", file=out)
    elif si.mask_atoms:
    ...
    print("Getting map as box", file=out)
    if hierarchy:
...
def get_target_boxes(si=None,ncs_obj=None,map=None,
  ...
  if pdb_inp:
    ...
    from scitbx.matrix import col
    ma=map.all()
    for i in range(i_start,i_end+1,i_step):
...
def run_local_sharpening(si=None,
  ...
  for ub,lb,centers_ncs_cart,center_cart in zip(
    ...
    print(80*"+", file=out)
    print("End of getting local sharpening for box %s" %(i), file=out)
    print(80*"+", file=out)
...
def auto_sharpen_map_or_map_coeffs(
    ...
    if si.solvent_fraction is None:
      si.solvent_fraction=get_iterated_solvent_fraction(
        ...
        map=map,
        out=out)
    print("Estimated solvent fraction: %s" %(si.solvent_fraction), file=out)
...
def optimize_k_sharpen_or_d_cut_or_b_iso(
  ...
  print("Current best score=%7.3f b_iso=%5.1f  k_sharpen=%5.1f d_cut=%5.1f" %(
       local_best_si.score,local_best_si.b_iso,
       local_best_si.k_sharpen,local_best_si.get_d_cut()), file=out)
  # existing values:
  value_dict={}
  ...
  for cycle in range(n_cycle_optimize):
    ...
    print("Current best score=%7.3f b_iso=%5.1f  k_sharpen=%5.1f d_cut=%5.1f" %(
     working_best_si.score,working_best_si.b_iso,
        working_best_si.k_sharpen,working_best_si.get_d_cut()), file=out)
        ...
        if id in value_dict:
          ...
          print(" %6.1f     %6.1f  %5s   %7.3f  %7.3f" %(
            ...
            "  %7.3f         %7.3f   %7.3f %7.3f " %(
             ...
             test_d_cut,test_k_sharpen), file=out)
  ...
  if working_best_si and working_best_si.score > local_best_si.score:
    print("Using new values of b_iso and k_sharpen and d_cut", file=out)
    local_best_si=working_best_si

  local_best_si.show_summary(out=out)

  return local_best_si 
...
def run_auto_sharpen(
  ...
  if si.auto_sharpen and (
    ...
    if pdb_inp:
      print("\nAuto-sharpening using model-defined box of density", file=out)
      max_box_fraction=si.max_box_fraction
    elif si.density_select_in_auto_sharpen:
      print("\nAuto-sharpening using density_select box of density", file=out)
      max_box_fraction=si.density_select_max_box_fraction
    else:
      print("\nAuto-sharpening using representative box of density", file=out)
      max_box_fraction=si.max_box_fraction
    ...
    if box_sharpening_info_obj is None: # did not do it
      ...
      print("Using box map to identify optimal sharpening", file=out)
  ...
  if si.adjust_region_weight and \
   ...
   for iii in range(1):  # just so we can break
    ...
    if min_region_weight >= max_region_weight:
      print("Region weight not found...", file=out)
      ok_region_weight=False
    ...
    if ok_region_weight and si.region_weight_method=='initial_ratio':
      ...
      if si.resolution:
        print("Using resolution-based b_iso (region_weight not found)", file=out)
        si.b_iso=si.get_target_b_iso()
        si.sharpening_method='b_iso_to_d_cut'
        auto_sharpen_methods=['b_iso_to_d_cut']
        print("b_iso=%7.2f" %(si.b_iso), file=out)
      else:
        si.region_weight=40
    ...
    for m in auto_sharpen_methods:
      ...
      for i in range(b_n):
        ...
        local_si=score_map(map_data=local_map_data,sharpening_info_obj=local_si,
          out=null_out())
        if m=='resolution_dependent':
          print("\nb[0]   b[1]   b[2]   SA   Kurtosis   sa_ratio  Normalized regions", file=out)
          print("\nB-sharpen   B-iso   k_sharpen   SA   "+\
             "Kurtosis  sa_ratio  Normalized regions", file=out)
          print(" %6.2f  %6.2f  %6.2f  " %(
            ...
            " %7.3f  %7.3f" %(
             local_si.sa_ratio,local_si.normalized_regions), file=out)
  ...
  if starting_mask:
    ...
    if not sites_cart: # pick top of map
      ...
      print("High point in map at (%7.2f %7.2f %7.2f) with value of %7.2f " %(
        sites_cart[0][0],sites_cart[0][1],sites_cart[0][1],value), file=out)
...
def run(args,
  ...
  if is_iteration:
    ...
    print("\nShifting map, model and NCS based on origin shift (if any)", file=out)
