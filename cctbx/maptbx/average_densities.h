  ...
  double const& d_min)
{
    ...
    int nz = map_data.accessor()[2];
    //int hkl_size = miller_indices.size();
    double two_pi = scitbx::constants::two_pi;
    //
    int maxtab = 10000;
    double dtable = two_pi / maxtab;
    ...
    //
    miller::index<> max_hkl = miller::index_span(miller_indices).max();
    af::shared<int> maxkl(max_hkl[2]+1);
    af::shared<af::shared<int> > maxhkl(max_hkl[1]+1);

    int mhkltot = 0;
    double sressq = 1.0 / (d_min*d_min);
    for (int il = 0; il < max_hkl[2]; il++) {
      double ssl = std::pow(il/abc[0], 2);
      maxkl[il] = abc[1] * std::sqrt(sressq - ssl);
      for (int ik = 0; ik < maxkl.size(); ik++) {
         double sskl = std::pow(ik/abc[1], 2) + ssl;
         maxhkl[ik].push_back(abc[0] * std::sqrt(sressq - sskl));
         mhkltot = mhkltot + maxhkl[ik][il] + 1;
      }
    }
    af::shared<std::complex<double> > result(mhkltot);
    af::shared<double> re_result;
    af::shared<double> im_result;

    re_result.resize(mhkltot, 0);
    im_result.resize(mhkltot, 0);


    double stepx = two_pi / nx;
    double stepy = two_pi / ny;
    double stepz = two_pi / nz;

    double dcx = std::cos(stepx);
    double dsx = std::sin(stepx);
    ...
    for (int il = 0; il < max_hkl[0]+1; il++) {
      for (int ik = 0; ik < maxkl[il]+1; ik++) {
        for (int ih = 0; ih < maxhkl[ik][il]+1; ih++) {
          all_indices.push_back(cctbx::miller::index<>(il,ik,ih));
    }}}
      ...
      for (int iy = 0; iy < ny; iy++) {
        ...
        for (int ix = 0; ix < nx; ix++) {
          if(map_data(ix,iy,iz)==1) {
            ...
            double* re_result_ = re_result.begin();
            ...
            for (int il = 0; il < max_hkl[0]+1; il++) {
               ...
               for (int ik = 0; ik < maxkl[il]+1; ik++) {
                  ...
                  for (int ih = 0; ih < maxhkl[ik][il]+1; ih++) {
                     ...
                     //sfre[iref] = sfre[iref] + sfrhkl;
                     //sfim[iref] = sfim[iref] + sfihkl;

                     result[iref] += std::complex<double>(sfrhkl,sfihkl);

                     //re_result[iref] += sfrhkl;
                     //im_result[iref] += sfihkl;



                     iref = iref + 1;
                     double sfrtmp = sfrhkl * delhc - sfihkl * delhs;
