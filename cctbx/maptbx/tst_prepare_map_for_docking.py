...
from cctbx.maptbx import box, prepare_map_for_docking
from prepare_map_for_docking import run_refine_cryoem_errors
from cctbx import adptbx
from scitbx.array_family import flex
import random

# Generate two half-maps with same anisotropic signal,
# different independent anisotropic noise. Test to see if
# simulation parameters are recovered.

# Start by working out how large the padding will have to be so that the
...
for i in range(6):
  assert(abs(b_refined_a[i]-b_expected[i]) < 30)
  assert(abs(b_refined_e[i]-2*b_target_e[i]) < 30)