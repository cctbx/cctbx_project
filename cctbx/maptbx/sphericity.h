        ...
        for(int ky = box_min[1]; ky <= box_max[1]; ky++) {
          ...
          for(int kz = box_min[2]; kz <= box_max[2]; kz++) {
            ...
            if(map_value>min_peak_value) {
              ...
              sphericity_tensor(0,0) += w*sc[0]*sc[0];
              sphericity_tensor(1,1) += w*sc[1]*sc[1];
              sphericity_tensor(2,2) += w*sc[2]*sc[2];
              sphericity_tensor(0,1) += w*sc[0]*sc[1];
              sphericity_tensor(0,2) += w*sc[0]*sc[2];
              sphericity_tensor(1,2) += w*sc[1]*sc[2];
            }
      }}}
