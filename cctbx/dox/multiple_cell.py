...
The situation is similar for the transformation from
a primitive trigonal or hexagonal setting to a triple cell
H setting.
...
very old papers. Unfortunately there is no established
notation for resolving the ambigous definition of IT-1983.
A related problem is that the Protein Data Bank (PDB)
sometimes (and sometimes not...) uses the symbol H to
distinguish between the hexagonal and the rhombohedral
setting of trigonal space groups (e.g. the symbol <i>H
3</i> in the PDB corresponds to <i>R 3 with hexangonal
axes</i> in IT-A1983). It could therefore cause significant
confusion if the sgtbx interprets the PDB symbols according
