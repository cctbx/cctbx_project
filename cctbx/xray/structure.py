...
class structure(crystal.special_position_settings):
  ...
  def scattering_dictionary_as_string(self):
    ...
    if self._scattering_type_registry is None:
      return result
    for tg in list(self._scattering_type_registry.as_type_gaussian_dict().items()):
      stype = tg[0]
      gaussian = tg[1]
      aa = "None"
      ab = "None"
  ...
  def scattering_types_counts_and_occupancy_sums(self):
    ...
    for scattering_type,unique_index in list(reg.type_index_pairs_as_dict().items()):
  ...
  def concatenate_inplace(self, other):
    ...
    for key1, item1 in zip(list(d1.keys()), list(d1.items())):
      for key2, item2 in zip(list(d2.keys()), list(d2.items())):
        if(key1 == key2):
          i1 = item1[1]
          i2 = item2[1]
          problem_flag = False
...
class meaningful_site_cart_differences(object):
  ...
  def show(self):
    import itertools
    for lbl, diff in zip(self.labels, self.delta):
