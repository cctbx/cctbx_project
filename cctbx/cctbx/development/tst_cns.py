...
def verify(sg_miller_set, sg_fcalc_set, sg_hl,
           p1_miller_indices, p1_fcalc, p1_hl):
  sgops = sg_miller_set.SgOps
  ...
  for p1_i in xrange(p1_miller_indices.size()):
    ...
    if (not are_similar_hl(sg_hl[sg_i], hl_asu)):
      print "Error:", sg_miller_set.SgInfo.BuildLookupSymbol()
      print sg_miller_set.H[sg_i]
      print "i:", sg_hl[sg_i]
      print "o:", hl_asu
      return # XXX
      raise AssertionError

def write_cns_input(miller_set, fcalc_set, hl):
  assert miller_set.H.size() == hl.size()
  cns_input = make_cns_input.unit_cell(miller_set.UnitCell)
  cns_input += make_cns_input.symmetry(miller_set.SgOps)
  l = cns_input.append
  l("xray")
  if (miller_set.friedel_flag):
    l("  anomalous=false")
  else:
    l("  anomalous=true")
  ...
  l("  end")
  l("  reflection")
  for i in xrange(miller_set.H.size()):
    l(  ("    index %d %d %d" % miller_set.H[i])
...
def exercise(SgInfo, d_min=2., friedel_flag=0, verbose=0):
  ...
  assert reflection_file.anomalous == (not sg_miller_set.friedel_flag)
  p1_miller_indices, p1_hl = reflection_file.join_hl_group()
  p1_fcalc_rso = reflection_file.reciprocal_space_objects["FCALC"]
  assert not miller.join_sets(p1_miller_indices, p1_fcalc_rso.H).have_singles()
  verify(sg_miller_set, sg_fcalc_set, sg_hl,
