...
class asu_clusters:
  ...
  def __init__(self, pair_asu_table, strictly_in_asu=True):
    n_sites = pair_asu_table.table().size()
    i_clusters = flex.size_t(n_sites, n_sites)
    clusters = shared.stl_vector_unsigned()
    for i_seq in xrange(n_sites):
      i_cluster = i_clusters[i_seq]
      if (i_cluster == n_sites):
        i_cluster = i_clusters[i_seq] = len(clusters)
        clusters.append(stl.vector.unsigned([i_seq]))
      cluster = clusters[i_cluster]
      for j_seq,j_sym_groups in pair_asu_table.table()[i_seq].items():
        j_cluster = i_clusters[j_seq]
        if (j_cluster == i_cluster): continue
        for j_syms in j_sym_groups:
          if (not strictly_in_asu or j_syms[0] == 0):
            if (j_cluster == n_sites):
              i_clusters[j_seq] = i_cluster
              cluster.append(j_seq)
            else:
              i_clusters.set_selected(cluster, j_cluster)
              clusters[j_cluster].extend(cluster)
              cluster = clusters[j_cluster]
              clusters[i_cluster] = stl.vector.unsigned()
              i_cluster = j_cluster
            break
    self.clusters = shared.stl_vector_unsigned()
    for cluster in clusters:
      if (cluster.size() > 0):
        self.clusters.append(cluster)

  def sort(self):
    sizes = flex.size_t([cluster.size() for cluster in self.clusters])
    perm = flex.sort_permutation(data=sizes, reverse=True)
    self.clusters = flex.select(sequence=self.clusters, permutation=perm)
    return self
