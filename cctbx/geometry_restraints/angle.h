...
namespace cctbx { namespace geometry_restraints {
  ...
  class angle : public cctbx::geometry::angle<double>
  {
    public:
      //! Default constructor. Some data members are not initialized!
      angle() {}
      ...
      {
        af::tiny<scitbx::vec3<double>, 3> grad_delta = d_angle_d_sites();
        if (!have_angle_model) {
          ...
          if (sin_angle_model < epsilon) {
            ...
            scitbx::vec3<double> d_angle_d_site0 = grad_delta[0] * pi_180;
            scitbx::vec3<double> d_angle_d_site1 = -grad_delta[1] * pi_180;
            scitbx::vec3<double> d_angle_d_site2 = grad_delta[2] * pi_180;
            double grad_factor = 2 * weight * delta_slack;
            for (std::size_t i=0; i<3; i++) {
              grads[i] = grad_factor * grad_delta[i];
            }
            if (curvs != 0)
            {
        ...
        for(int i=0;i<3;i++) {
          if ( sym_ops.get() != 0 && !sym_ops[i].is_unit_mx() ) {
            scitbx::mat3<double> r_inv_cart_ = r_inv_cart(
              unit_cell, sym_ops[i]);
            gradient_array[i_seqs[i]] += r_inv_cart_ * grads[i];
          }
        ...
        angle_proxy const& proxy) const
      {
        ...
        optional_container<af::shared<sgtbx::rt_mx> > const&
          sym_ops = proxy.sym_ops;
        af::tiny<scitbx::vec3<double>, 3> grads = d_angle_d_sites();
