...
class Target:
  def __init__(self, path_to_integration_dir, out_name=''):
  ...
  def print_ucs(self):
    if self.out_prefix != '':
      outfile = "{}_niggli_ucs".format(self.out_prefix)
    else:
      outfile = "niggli_ucs"
  ...
  def cluster_PCA(self, threshold, method, linkage_method):
  ...
  def cluster(self, threshold, method, linkage_method):
    ...
    g6_cells = np.array([make_G6(n_cell)
    ...
    pair_distances = dist.pdist(g6_cells,
                                metric=lambda a, b: find_distance(
                                  a, b, dist_method))
    ...
    self.this_linkage = hcluster.linkage(pair_distances,
                                    ...
                                    metric=lambda a, b: find_distance(
                                      a, b, dist_method))
    clusters = hcluster.fcluster(self.this_linkage,
                                 threshold,
                                 criterion='distance')

    # 3. print out some information that is useful.
    print "{} clusters have been identified.".format(max(clusters))
    ...
    for cluster in range(max(clusters)):
      this_cluster_files = [self.names[i]
      ...
      if len(this_cluster) != 1:
        # Print a file with the cluster members
        outfile = "{}_cluster_{}.list".format(self.out_prefix, cluster + 1)
        with open(outfile, 'wb') as thefile:
          for item in this_cluster_files:
            print>>thefile, item

        pg_strings = ["{} images in {}".format(all_pgs[pg], pg)
                      for pg in all_pgs]
        point_group_string = ", ".join(pg_strings) + "."
        print "".join([("{:^5} {:^14} {:<5.1f}({:<4.1f}) {:<5.1f}({:<4.1f})"
                        " {:<5.1f}({:<4.1f})").format(
          cluster,
          len(this_cluster),
          ...
          np.median(this_cluster[:, 2]), np.std(this_cluster[:, 2])),
                       (" {:<6.2f}({:<4.2f}) {:<6.2f}"
                        "({:<4.2f}) {:<6.2f}({:<4.2f})").format(
                         ...
                         np.std(this_cluster[:, 3]),
                         np.median(this_cluster[:, 4]),
                         np.std(this_cluster[:, 4]),
                         np.median(this_cluster[:, 5]),
                         np.std(this_cluster[:, 5]))
        ])
        print "  --> " + point_group_string
      else:
        singleton_files.append(this_cluster_files)
    ...
    # Print a file with the singletons
    outfile = "{}_singletons.list".format(self.out_prefix)
    with open(outfile, 'wb') as thefile:
      for item in singleton_files:
        print>>thefile, item[0]


def find_distance(G6a, G6b, key):
  ...
  if plot_ucs:
    ...
    for i in range(len(ucs.niggli_ucs)):
      ax.scatter(np.array(ucs.niggli_ucs)[i, 0],
                 ...
                 c=cmap(ucs.clusters[i - 1] / max(ucs.clusters)),
                 marker='o', s=20)
