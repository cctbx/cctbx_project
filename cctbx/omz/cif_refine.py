...
class extract_from_cif_files(object):
  ...
  def process(O, report_id, refl_file, refl_cif, model_file, model_cif):
    ...
    def get_cs(cif, buffer):
      for cif_block in list(cif.model().values()):
    ...
    print("."*79)
    #
    O.xray_structure = list(cctbx.xray.structure.from_cif(
  ...
  def process_geom_bond(O, model_cif):
    xs = O.xray_structure
    scs = xs.scatterers()
    i_seq_by_lbl = dict(list(zip(scs.extract_labels(), list(range(scs.size())))))
    if (len(i_seq_by_lbl) != scs.size()):
      return None
    edge_set = set()
    for cif_block in list(model_cif.model().values()):
