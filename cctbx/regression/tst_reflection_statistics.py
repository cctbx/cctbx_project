...
def exercise_double_coset_decomposition(
  ...
  double_coset_repetitions = [len(v) for v in list(double_coset_ccs.values())]
  ...
  if (failure):
    for cc,ops in list(double_coset_ccs.items()):
      if (len(ops) > 1):
        print(cc, ops)
        for op in ops:
          ri = sgtbx.rt_mx(op).r().info()
