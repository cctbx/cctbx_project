...
class indexing_ambiguity_handler(object):
  ...
  def should_terminate(self, iparams, pickle_filename):
    ...
    idah = indamb_handler()
    alt_dict = idah.get_observations(pickle_filename, iparams)
    if len(list(alt_dict.keys())) == 1 and iparams.indexing_ambiguity.mode != 'Forced':
      return True
  ...
  def run(self, args):
    ...
    frames = [(i, frame_files[i], iparams) for i in random.sample(list(range(n_frames)), iparams.indexing_ambiguity.n_sample_frames)]
    ...
    if n_frames > iparams.indexing_ambiguity.n_selected_frames:
      ...
      scaled_pres_set = scale_frames(list(range(len(frame_files_processed))), frame_files_processed, iparams)
