...
The implementation is nearly entirely done in Python, and reside in the module\\ \code{smtbx.ab_initio.charge_flipping}. It features the original method as well as the newer one enhanced to deal with weak reflections~\cite{Oszlanyi:2004}. Here is the starting point to explore these tools
...
It should be noted that \eqnref{optimalK} is used in the \cctbx too but in the context of LBFGS minimisation, in the \cpp class \code{ls_with_scale} defined in \code{cctbx/xray/targets.h}. Because of the remark just made about first-order derivatives, this choice of $K$ is totally invisible from the point of view of LBFGS.
...
As described in a previous \cctbx newsletter \cite{cctbxnews:2004b}, geometry restraints of the kind that are extensively used in macromolecular crystallography have already been implemented in the \cctbx.  However, with the exception of the bond distance restraint, these were not able to deal with symmetry equivalent atoms, as is allowed by most common small molecule refinement programs.  These have now been extended to allow symmetry equivalent atoms, and a new bond similarity restraint has been added.  We have also implemented several common restraints on anisotropic displacement parameters (ADPs), including restraints based on Hirshfeld's ``rigid-bond'' test \cite{Hirshfeld:1976}, similarity restraints and isotropic restraints.

\subsubsection{Restraints on Atomic Displacement Parameters}

There appears to very little in the literature with regard to restraints on ADPs, and in particular the details of their implementation in refinement programs. It was therefore necessary to devise our formulae for the equations of restraints and derive their gradients with respect to the least squares parameters. The correctness of the analytical gradients was confirmed by testing against gradients determined by the finite differences method. The residuals were also tested for frame invariance (\textit{i.e.} the residual should be invariant with respect to the orientation of the ADP within the Cartesian frame).
...
Here we minimise the difference between the Cartesian ADPs, $\mat{U}_{cart}$, and the isotropic equivalent, $\mat{U}_{eq}$. As in section \ref{ADP:similarity}, we must remember that we are dealing with symmetric matrices, and we can therefore define the weighted least squares residual as
\begin{equation}
...
\end{equation}
The derivatives of the residual with respect to the off-diagonal elements are
\begin{equation}
\partialder{R}{U_{ij,i\neq j}} = 4 w U_{ij},
\end{equation}
and the derivatives with respect to the diagonal elements are
\begin{equation}
