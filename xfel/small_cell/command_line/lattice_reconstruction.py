...
#      print("\nTo reproduce these clusters, add to phil:")
#      print("initialization.triangle_finding.interactive.gmm_params {")
#      print("  q1_centers = " + " ".join(str(c['q1_center']) for c in cluster_results))
#      print("  q2_centers = " + " ".join(str(c['q2_center']) for c in cluster_results))
#      print("  radii = " + " ".join(str(c['radius']) for c in cluster_results))
#      print("  theta_centers = " + " ".join(str(c['theta_center']) for c in cluster_results))
#      print("  theta_widths = " + " ".join(str(c['theta_width']) for c in cluster_results))
#      print("}")
#
#    # After collecting all cluster_results
#    if not cluster_results:
#      raise Sorry("No clusters selected")
#    
#    print(f"\nFitting Gaussian mixture model with {len(cluster_results)} components")
#
#    # Pre-filter points for GMM fitting
#    mask = np.zeros(len(self.triplets), dtype=bool)
#    safety = self.params.initialization.triangle_finding.interactive.cluster_safety_margin
#
#    for cluster in cluster_results:
#      q1, q2 = cluster['q1_center'], cluster['q2_center']
#      r = cluster['radius'] * safety
#      nearby = ((1/self.triplets[:,1] - q1)**2 +
#                (1/self.triplets[:,2] - q2)**2 <= r**2)
#      theta = self.triplets[:,3]
#      theta_center = cluster['theta_center']
#      theta_width = cluster['theta_width']
#      theta_nearby = (abs(theta - theta_center) <= safety * theta_width/2)
#      mask |= (nearby & theta_nearby)
#
#    # Store indices of points we're using
#    self.sampled_indices = np.where(mask)[0]
#    print(f"\nSelected {len(self.sampled_indices)} points for GMM fitting")
#
#    X = np.column_stack((
#      1/self.triplets[self.sampled_indices,1],
#      1/self.triplets[self.sampled_indices,2],
#      self.triplets[self.sampled_indices,3]
#    ))
#
#    # Add noise component to means and covariances
#    n_components = len(cluster_results) + 1  # Add one for noise
#
#    # Add noise component with wide spread
#    means_init = np.vstack([
#      [[c['q1_center'], c['q2_center'], c['theta_center']] for c in cluster_results],
#      # Center noise component in middle of data range
#      [X.mean(axis=0)]
#    ])
#
#    covariances_init = []
#    for cluster in cluster_results:
#      var_q = (cluster['radius']/2)**2
#      var_theta = (cluster['theta_width']/4)**2
#      cov = np.diag([var_q, var_q, var_theta])
#      covariances_init.append(cov)
#    # Add wide covariance for noise component
#    noise_var = np.var(X, axis=0) * 10  # Make noise component very wide
#    covariances_init.append(np.diag(noise_var))
#
#    # Initialize with higher weight for noise component
#    weights_init = np.ones(n_components)
#    weights_init[-1] = 3.0  # Give noise component higher initial weight
#    weights_init = weights_init / weights_init.sum()
#
#    gmm = GaussianMixture(
#      n_components=n_components,
#      means_init=means_init,
#      precisions_init=np.array([np.linalg.inv(c) for c in covariances_init]),
...
#    gmm.fit(X)
#
#    # Store results using only the selected points
#    self.cluster_data = {
...
#      print(f"\nCluster {i}:")
#      print(f"  Center (q1, q2, theta): {gmm.means_[i]}")
#      print(f"  Weight: {gmm.weights_[i]:.3f}")
#      # Convert q back to d for printing
#      d1, d2 = 1/gmm.means_[i,0], 1/gmm.means_[i,1]
#      theta = gmm.means_[i,2]
#      print(f"  Center (d1, d2, theta): ({d1:.2f}Å, {d2:.2f}Å, {theta:.1f}°)")
#
#    # Store centers and covariances
#    self.cluster_centers = {}
#    self.cluster_covariances = {}
#    for i in range(n_components):
#      self.cluster_centers[i] = gmm.means_[i]
#      self.cluster_covariances[i] = gmm.covariances_[i]
  ...
  def cluster_triplets_orig(self, eps=0.005, min_samples=10, max_points=50000, dmin=None, dmax=None):
    """Find clusters in reciprocal space

    Args:
      eps: DBSCAN epsilon parameter in scaled space
      min_samples: Minimum points to form a cluster
      max_points: Maximum points to sample for clustering
      dmin, dmax: Optional d-spacing range to analyze
    """
    # Convert to q-vector magnitudes
    q1 = 1/self.triplets[:,1]  # |q1|
    q2 = 1/self.triplets[:,2]  # |q2|
    theta = self.triplets[:,3]  # degrees

    # Start with all indices
    selected = np.arange(len(q1))
    
    # Apply d-spacing filter if requested
    if dmin is not None or dmax is not None:
      dmin = dmin or 0
      dmax = dmax or float('inf')
      mask = ((1/q1 >= dmin) & (1/q1 <= dmax) &
    ...
    # Scale angles to make differences comparable to q-vector differences
    angle_scale = (np.pi/180) * 0.2

    points = np.column_stack((q1, q2, theta * angle_scale))
    ...
    for k in set(clustering.labels_) - {-1}:
      mask = clustering.labels_ == k
      cluster_points = points[mask]

      center_scaled = np.mean(cluster_points, axis=0)
      self.cluster_centers[k] = (
        center_scaled[0],           # q1
        center_scaled[1],           # q2
        center_scaled[2]/angle_scale  # theta (degrees)
      )
      self.cluster_covariances[k] = np.cov(cluster_points, rowvar=False)

  def plot_clusters(self):
    """Create pairwise plots of clustering results"""
    ...
    # Get data
    q1 = 1/self.triplets[self.sampled_indices,1]
    q2 = 1/self.triplets[self.sampled_indices,2]
    theta = self.triplets[self.sampled_indices,3]
    labels = self.cluster_data['labels']

    # Plot q1 vs q2
    noise = labels == -1
    ax1.scatter(q1[noise], q2[noise], c='gray', alpha=0.1, s=1)

    for k in set(labels) - {-1}:
      mask = labels == k
      ax1.scatter(q1[mask], q2[mask], alpha=0.5, s=1, label=f'Cluster {k}')
    ...
    # Plot q1 vs theta
    ax2.scatter(q1[noise], theta[noise], c='gray', alpha=0.1, s=1)
    for k in set(labels) - {-1}:
      mask = labels == k
      ax2.scatter(q1[mask], theta[mask], alpha=0.5, s=1)

    ax2.xaxis.set_major_formatter(tick.FuncFormatter(q_to_d))
    ax2.set_xlabel('d1 (Å)')
    ax2.set_ylabel('Angle (degrees)')

    # Plot q2 vs theta
    ax3.scatter(q2[noise], theta[noise], c='gray', alpha=0.1, s=1)
    for k in set(labels) - {-1}:
      mask = labels == k
      ax3.scatter(q2[mask], theta[mask], alpha=0.5, s=1)
