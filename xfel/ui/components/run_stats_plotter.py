...
def plot_run_stats(stats, d_min, interactive=True, xsize=30, ysize=10):
  ...
  for idx in xrange(len(run_numbers)):
    ...
    ax4.text(start_t, .9, "run %d" % run_numbers[idx]).set_fontsize(10)
    ax4.text(start_t, .7, "%d f/%d h" % (lengths[idx], n_hits)).set_fontsize(10)
    ax4.text(start_t, .5, "%d (%d) idx" % (n_idx_low, n_idx_high)).set_fontsize(10)
    ax4.text(start_t, .3, "%-3.1f%% hit" % (100*n_hits/lengths[idx],)).set_fontsize(10)
    ax4.text(start_t, .1, "%-3.1f (%-3.1f)%% idx" % \
      (100*n_idx_low/lengths[idx], 100*n_idx_high/lengths[idx])).set_fontsize(10)
    ax4.set_xlabel("timestamp (s)\n# images shown as all (%3.1f Angstroms)" % d_min).set_fontsize(10)
    ax4.set_yticks([])
    for item in [ax1, ax2, ax3, ax4]:
      item.tick_params(labelsize=10)
    start += lengths[idx]
  if interactive:
    plt.show()
  else:
    f.set_size_inches(xsize, ysize)
    f.savefig("runstats_tmp.png", bbox_inches='tight', dpi=50)
    plt.close(f)
