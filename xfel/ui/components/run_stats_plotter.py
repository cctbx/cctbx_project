...
def plot_run_stats(stats, d_min, interactive=True):
  ...
  for idx in xrange(len(run_numbers)):
    ...
    ax4.text(start_t, .7, "%d f/%d h" % (lengths[idx], n_hits))
    ...
    ax4.set_xlabel("timestamp (s)\n# images shown as all (%3.1f Angstroms)" % d_min)
    ax4.set_yticks([])
    start += lengths[idx]
  if interactive:
    plt.show()
