...
debug=False # set debug=True, if you want to see a per-rank json/pickle file list generated by this calculator
...
class file_load_calculator(object):
  ...
  def calculate_file_load_node_memory_based(self, available_rank_count):
    '''Assign json/pickle file pairs to nodes taking into account the node memory limit. Then distribute node-assigned file pairs over the ranks within each node. Return a dictionary {rank:file_list}'''
    ...
    for node in range(required_number_of_nodes):
      rank_base = (node * self.params.input.parallel_file_load.node_memory.ranks_per_node)
      for rank in range(rank_base, rank_base + self.params.input.parallel_file_load.node_memory.ranks_per_node):
        rank_files[rank] = node_files[node][rank - rank_base::self.params.input.parallel_file_load.node_memory.ranks_per_node]
...
class Script(Script_Base):
  ...
  def get_file_list(self):
    ...
    file_list = file_lister(self.params).filename_lister()
    print("Built an input list of %d json/pickle file pairs"%len(file_list))
    print("You can view a list of json/pickle file pairs per rank, if you set debug=True")
    return file_list
