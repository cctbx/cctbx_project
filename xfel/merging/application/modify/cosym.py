...
class cosym(worker):
  ...
  def runold(self, experiments, reflections):
    if self.mpi_helper.rank == 0:
      print("Starting cosym worker")
      P = Profiler("cosym all time")
    ...
    def task_1(uuid_starting=[], mpi_helper_size=1, do_plot=False):
      ...
      if mpi_helper_size == 1: # simple case, one rank
       for experiment in all_sampling_experiments:
        ...
        values = token_passing_left_right((experiments,reflections),self.mpi_helper.comm)
  ...
  def runnew(self, input_experiments, input_reflections):
    from collections import OrderedDict
    if self.mpi_helper.rank == 0:
      print("Starting cosym worker")
