...
class reflection_filter(worker):
  ...
  def validate(self):
    ...
    if filter_by_target_energy:
      check0 = self.params.select.target_energy.target_eV > 0
      check1 = self.params.select.target_energy.n_sigma > 0
      assert check0 and check1, \
        'target_eV and n_sigma must be > 0'
  ...
  def run(self, experiments, reflections):
    ...
    if (not filter_by_significance) and (not filter_by_isolation_forest) and (not filter_by_target_energy):
  ...
  def apply_target_energy_filter(self, experiments, reflections):
    ...
    target_E = self.params.select.target_energy.target_eV  # eV
