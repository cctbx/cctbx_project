...
def coerce(data):
  for key in data:
    print("datamgr",key,data[key].shape)
...
"""
    ...
    for iexp, (cexpt, _) in enumerate(reflection_table_utils.iterate_experiments_and_load_imagesets(all_experiments)):
      print("   Looking for",cexpt.imageset.paths()[0],"#",cexpt.imageset.indices()[0],"among",cached_imageset.indices().size(),"expt",iexp)
      spectrum = cached_imageset.get_spectrum(cexpt.imageset.indices()[0])

      plt.plot(spectrum.get_energies_eV(), spectrum.get_weights(), '-')

    plt.xlabel("Energy (eV)")
    plt.ylabel("Weights")
    plt.title("Rank",rank,"All images len",len(all_experiments))
    plt.show()
    # NKS
"""
