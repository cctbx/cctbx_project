...
#
# LIBTBX_SET_DISPATCHER_NAME cctbx.xfel.time_varying_refinement
#
...
time_varying_refinement {
  ...
  respect_rungroup_barriers = True
    .type = bool
    .help = "Enforce separation by rungroup at time of chunking (default) or striping."
  ...
  def run(self):
    ...
    for d in self.dirname, self.intermediates, self.extracted:
      if not os.path.isdir(d):
        os.mkdir(d)
    if self.params.time_varying_refinement.output_folder is None:
      self.params.time_varying_refinement.output_folder = os.getcwd()
