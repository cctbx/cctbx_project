...
## 2026-07-21
Fix `tst_rna_validate.py` crashing instead of skipping when `phenix_regression` is unavailable. `exercise_1` returns None on the skip path, following the convention every other test in the directory uses, but this is the only test that consumes the return value and it did so unguarded, so a checkout without `phenix_regression` raised AttributeError and looked like a genuine regression. Guarded the `exercise_pdbvcif` call. Verified both ways: it now skips cleanly without the data, and with the data present the full test runs and passes.
