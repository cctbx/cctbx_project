...
class mtz_handler(object):
  ...
  def format_miller_arrays(self, iparams):
    ...
    #get only indices available in the HL miller_array
    miller_indices_hl = miller_arrays[ind_miller_array_hl].indices()

    #filter out any indices that don't match.
    matches = miller.match_multi_indices(
                  miller_indices_unique=miller_indices_hl,
                  miller_indices=miller_arrays[0].indices())

    miller_indices = flex.miller_index([miller_indices_hl[pair[0]] for pair in matches.pairs()])
    flex_fp = flex.double([flex_fp_all[pair[1]] for pair in matches.pairs()])
    flex_sigmas = flex.double([flex_sigmas_all[pair[1]] for pair in matches.pairs()])
    flex_phib = flex.double([flex_phib_all[pair[0]] for pair in matches.pairs()])
