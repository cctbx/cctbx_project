...
class ProcWindow(wx.Frame):
  def __init__(self, parent, id, title, params):
    ...
    self.log_window = wx.TextCtrl(self, style=wx.TE_MULTILINE | wx.TE_READONLY | wx.TE_DONTWRAP,
                                  size=self.log_box.GetSize())
    self.log_window.SetFont(wx.Font(8, wx.TELETYPE, wx.NORMAL, wx.NORMAL, False))
    self.log_sizer.Add(self.log_window, flag=wx.EXPAND | wx.LEFT | wx.RIGHT | wx.BOTTOM | wx.TOP, border=5)
    self.main_sizer.Add(self.log_sizer, flag=wx.EXPAND | wx.LEFT | wx.RIGHT | wx.BOTTOM, border=15)
    ...
    self.main_sizer.Add(self.gauge_process, flag=wx.EXPAND | wx.LEFT | wx.RIGHT, border=10)
  ...
  def onFinishedOneTask(self, e):
    ...
    if os.path.isfile(self.tmp_abort_file):
      ...
      self.sb.SetStatusText('{} of {} images imported'.format(len(found_files), len(self.init.input_list)), 1)
    elif self.stage == 'gs_import':
      self.sb.SetStatusText('{} of {} objects imported'.format(len(found_files), len(self.init.gs_img_objects)), 1)
    elif self.stage == 'process':
      self.sb.SetStatusText('{} of {} images processed, {} successfully integrated'\
                            ''.format(len(found_files), len(self.img_objects), len(processed_images)), 1)
