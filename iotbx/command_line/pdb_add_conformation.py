...
def run (args, out=sys.stdout) :
  ...
  for arg in args :
    if os.path.isfile(arg) :
      ...
      if (f.file_type == "pdb") :
        ...
        user_phil.append(libtbx.phil.parse("pdb_file=\"%s\"" % f.file_name))
  ...
  if (params.new_occ is None) or (params.new_occ < 0) or (params.new_occ > 1) :
      ...
      for residue_group in chain.residue_groups() :
        ...
        for atom_group in residue_group.atom_groups() :
          ...
          if (flags.count(1) > 0) :
            ...
            new_group.altloc = params.new_altloc
            if (flags.count(0) > 0) :
              j_atom = 0
              k_atom = 0
              ...
              for old_atom, new_atom in zip(old_atoms, new_group.atoms()) :
                old_occ = atom.occ - params.new_occ
                if (old_occ == 0) :
                  ...
                  raise Sorry("Atom occupancy dropped below zero:\n" +
                    ...
                    "too high.")
                old_atom.set_occ(atom.occ - params.new_occ)
