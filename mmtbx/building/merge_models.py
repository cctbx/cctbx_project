...
def get_crossover_dict(
        ...
        for chain2 in model2.chains():
          ...
          for i in range(xyz1.size()):
            ...
            if dd<= dist_max_sq:  # can crossover here
              if not i in list(crossover_dict.keys()): crossover_dict[i]={}
              if not model1.id in list(crossover_dict[i].keys()):
                crossover_dict[i][model1.id]=[]
              if not model2.id in list(crossover_dict[i].keys()):
  ...
  if minimum_matching_atoms > 2:
    ...
    for i in range(n_residues):
      if not i in list(crossover_dict.keys()): continue
      for id1 in list(crossover_dict[i].keys()):
        for id2 in crossover_dict[i][id1]:
          ...
          for offset in offset_range:
            ...
            if not id2 in crossover_dict.get(i1,{}).get(id1,[]):
              if not i in list(delete_dict.keys()): delete_dict[i]={}
              if not id1 in list(delete_dict[i].keys()): delete_dict[i][id1]=[]
  ...
  for pos in [0,n_residues-1]:
    if not pos in list(crossover_dict.keys()):
      crossover_dict[pos]={}
    for id1 in used_model_ids:
      for id2 in used_model_ids:
        if id1==id2: continue
        if not id1 in list(crossover_dict[pos].keys()):
...
def get_cc_dict(hierarchy=None,crystal_symmetry=None,
  ...
  std_size=None
  for model_id in list(cc_dict.keys()):
