...
class xtb_manager(base_qm_manager.base_qm_manager):
  ...
  def get_hessian_filename(self):
    # Full Cartesian Hessian from a --hess run (namespaced), Hartree/Bohr^2.
    return f'xtb_{self.preamble}.hessian'
  ...
  def get_hessian_input_lines(self):
    """xcontrol for a ``--hess`` run: charge/spin/GFN only, no ``$opt`` or
    ``$fix``. The Hessian is evaluated at the geometry as given."""
    gfn = self._get_gfn()
    lines = [
      f'# xtb_manager hessian input ({self.preamble})',
      f'# gfn={gfn} solvent={self._get_solvent() or "none"} '
      f'threads={self._get_threads()} runtype=hess',
      f'$chrg {int(self.get_charge())}',
      f'$spin {self.get_multiplicity() - 1}',
    ]
    if gfn != 'ff':
      lines += ['$gfn', f'   method={gfn}', '$end']
    return '\n'.join(lines) + '\n'

  def get_xyz_lines(self, sites_cart=None):
    """The standard .xyz file xTB reads (full precision), at the manager's atom
    coordinates or, when *sites_cart* is given, at those (same atom order)."""
    ...
    for i, atom in enumerate(self.atoms):
      ...
      x, y, z = atom.xyz if sites_cart is None else sites_cart[i]
  ...
  def get_cmd(self, gradients_only=False, hessian=False):
    exe = get_exe()
    if not exe:
  ...
  def run_cmd(self, redirect_output=True, log=None, hessian=False):
  ...
  def run_hessian(self, sites_cart=None, hessian_atoms=None, partial=True, log=None):
    ...
    *hessian_atoms* / *partial* give signature parity with
    ...
    if os.path.exists(hessian_filename):
      os.remove(hessian_filename)
    self.run_cmd(log=log, hessian=True)
    if not os.path.exists(hessian_filename):
      raise Sorry(f'xtb --hess did not produce {hessian_filename}; '
                  f'check {self.get_log_filename()}')
    return os.path.abspath(hessian_filename)
