...
def from_map_map_atoms_optimal_radius(map_1, map_2, sites_cart, unit_cell):
  radii_coarse = [r/100. for r in range(100,950,50)]
  cc_best = -999
  r_best = None
  for r in radii_coarse:
    ...
    if(cc>cc_best):
      r_best = r
      cc_best = cc
  r_fine = [r/100. for r in range(int(r_best*100)-50,int(r_best*100)+50,10)]
  for r in r_fine:
    cc = from_map_map_atoms(map_1=map_1, map_2=map_2, sites_cart=sites_cart,
      unit_cell=unit_cell, radius=r)
    if(cc>cc_best):
