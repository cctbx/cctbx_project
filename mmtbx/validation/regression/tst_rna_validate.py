...
def exercise_1(test_mmcif=False):
  ...
  assert len(rv.angles.results) == 47, len(rv.angles.results)
  ...
  assert len(angles_json["flat_results"]) == 47, "tst_rna_validate json output not returning correct number of angle outliers, changed to: " + str(len(angles_json["flat_results"]))
  ...
  assert test_utils.count_dict_values(angles_json['hierarchical_results'], [1.0, 1.0, 1.0])==47, "tst_rna_validate json hierarchical output number of angle outliers changed to: "+str(test_utils.count_dict_values(angles_json['hierarchical_results'], [1.0, 1.0, 1.0]))
