...
"""
    if (self.restraints is not None) :
      kin_txt += self.restraints.as_kinemage() + "\n"
    if (self.cbetadev is not None) :
      kin_txt += self.cbetadev.as_kinemage() + "\n"
    if (self.ramalyze is not None) :
      kin_txt += self.ramalyze.as_kinemage() + "\n"
    return kin_txt
  ...
  def write_coot_script (self, file_name) :
    ...
    if (self.clashscore is not None) :
      ...
      if (self.clashscore.probe_file is not None) :
        f.write("handle_read_draw_probe_dots_unformatted(\"%s\")\n" %
