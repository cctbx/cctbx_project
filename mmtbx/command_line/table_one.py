...
class _(oop.injector, molprobity.molprobity):
  ...
  def as_table1_column(self,
    ...
    return iotbx.table_one.column(
      ...
      # model properties
      n_atoms=self.model_stats.result().n_atoms - self.model_stats.result().n_hd,
      n_macro_atoms=self.model_stats.result().n_protein_atoms + self.model_stats.result().n_nucleotide_atoms,
      n_ligand_atoms=self.model_stats.result().n_other_atoms,
      n_waters=self.model_stats.result().n_water_atoms,
      n_residues=self.model_stats.result().n_protein,
