...
def run(args, log=None, ccp4_map=None,
  ...
  #
  m = ccp4_map
  if(m.unit_cell_crystal_symmetry().space_group().info().type().number()> 1):
    raise Sorry("Input map space group: %d. Must be P1."%m.unit_cell_crystal_symmetry().space_group().info().type().number())
