...
class segment_library(object):
  ...
  def set_up_indexing(self):
    ...
    for segment in self.segments:
      ...
      for index in index_list:
        if not index in list(index_dict.keys()):
          index_dict[index]=[]
  ...
  def get_xyz_in_order(self,xyz=None,original_order=None,
    ...
    recover_target_order=list(range(len(target_order)))
    recover_sort_list=[]
...
class connected_group_segment(object):
...
class connected_group(object):
  ...
  def get_connection_rms(self,s1,s2,
    ...
    if not s1.segment in list(self.connection_rms_dict.keys()):
      self.connection_rms_dict[s1.segment]={}
    if not s2.segment in list(self.connection_rms_dict[s1.segment].keys()):
...
class replacement_segment_summary(object):
...
class replace_with_segments_from_pdb(object):
      ...
      if ok:
        ...
        for j in range(i+1,len(connected_groups)):
          ...
          for k in range(cg.get_left_connection(),cg.get_right_connection()+1):
            if k in list(cg_dict[cg_j].keys()):
              cg_dict[cg_j][k]=True
  ...
  def get_connected_groups_from_segments(self,params=None,
    ...
    for rs in replacement_segments:
      ...
      for lc,rc in connection_pairs:
          ...
          connected_groups.append(cg)
          if not lc in list(possible_right_connection_dict.keys()):
            possible_right_connection_dict[lc]=[]
          possible_right_connection_dict[lc].append(cg)
          if not rc in list(possible_left_connection_dict.keys()):
