...
def step_j(h_o, log):
  ...
  #assert V_V_[0] != 0 and V_V_[1] != 0 and V_V_[2] != 0
  tx0, ty0, tz0 = 0, 0, 0
...
class decompose_tls(object):
  ...
  def eigen_system_default_handler(self, m, eps=1.e-9):
    ...
    print >> self.log, list(vals)
    print >> self.log, list(vecs)
    ...
    if(abs(vals[0]-vals[1])>=eps and
      ...
      print >> self.log, "  eigen values:", list(vals)
      print >> self.log, "  eigen vectors:"
      print >> self.log, "   l_1:", l_1
      print >> self.log, "   l_2:", l_2
      print >> self.log, "   l_3:", l_3
      l_x, l_y, l_z = l_3, l_2, l_1
      vals = [vals[2], vals[1], vals[0]]
      print >> self.log, "re-assign: l_x, l_y, l_z = l_3, l_2, l_1"
      print >> self.log, "  eigen values:", list(vals)
      print >> self.log, "  eigen vectors:"
      print >> self.log, "   l_x:", l_x
      print >> self.log, "   l_y:", l_y
      print >> self.log, "   l_z:", l_z
  ...
  def step_a(self, T,L,S):
    ...
    print >> self.log, "  b:\n", b
    m_inv = m.inverse()
    print >> self.log, "m_inverse:\n", m_inv
    p = m_inv * b
    P = matrix.sqr((
          0, p[2], -p[1],
      -p[2],    0,  p[0],
       p[1], -p[0],    0))
    print >> self.log, "  vector p:\n", p
    print >> self.log, "  matrix P:\n", P
  ...
  def step_b(self, T_p, L_p, S_p, eps = 1.e-6):
    ...
    R_PL = matrix.sqr(
      [l_x[0], l_y[0], l_z[0],
       l_x[1], l_y[1], l_z[1],
       l_x[2], l_y[2], l_z[2]])
    print >> self.log, "  rotation matrix R_PL:\n", R_PL
  ...
  def step_c(self, T_L, L_L, S_L, eps = 1.e-6):
    ...
    L_ = L_L.as_sym_mat3()
    Lxx, Lyy, Lzz = L_[0], L_[1], L_[2]
    print >> self.log, "Lxx, Lyy, Lzz:", Lxx, Lyy, Lzz
    wy_lx=0
    wz_lx=0
    wx_ly=0
    wz_ly=0
  ...
  def step_d(self, S_L, L_L, T_L, eps=1.e-6):
    ...
    if (1):
      ...
      sqrt = math.sqrt
      tmin = max(Sxx-sqrt(Txx*Lxx), Syy-sqrt(Tyy*Lyy), Szz-sqrt(Tzz*Lzz))
      tmax = min(Sxx+sqrt(Txx*Lxx), Syy+sqrt(Tyy*Lyy), Szz+sqrt(Tzz*Lzz))
  ...
  def step_e(self, S_C, L_L, eps=1.e-9):
    ...
    Lxx, Lyy, Lzz = L_[0], L_[1], L_[2]
    print >> self.log, "Lxx, Lyy, Lzz:   ", Lxx, Lyy, Lzz
    print >> self.log, "SCxx, SCyy, SCzz:", S_C[0], S_C[4], S_C[8]
