...
def run(args, command_name = "phenix.tls"):
  ...
  #
  chain_ids = []
  for model in mmtbx_pdb_file.processed_pdb_file.all_chain_proxies.pdb_hierarchy.models():
  ...
  if(len(xsfppf.xray_structures) > 1):
    raise Sorry("Cannot handle PDB file with multiple MODEL.")
  xray_structure = xsfppf.xray_structures[0]
  utils.print_header("Input model summary", out = log)
  ...
  tls_groups = iotbx.pdb.remark_3_interpretation.extract_tls_parameters(
  ...
  if(len(params.selection) > 0): tls_selections_strings = params.selection
