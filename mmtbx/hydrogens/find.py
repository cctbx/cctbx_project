...
def extract_hoh_peaks(peaks, pdb_hierarchy, pdb_atoms, xray_structure):
  ...
  for i_seq,ag in list(o_i_seq_ag.items()):
    ag_atoms = ag.atoms()
...
def len_peaks(set_of_peaks):
  r = 0
  for peaks in list(set_of_peaks.values()):
    r += len(peaks)
  return r
...
def build_water_hydrogens_from_map(model, fmodel, params=None, log=None):
  ...
  print("Peaks to consider: ", len(list(pks.keys())), " : ", len(list(pkss.keys())), file=log)
