...
def flip_residue(residue, mon_lib_srv=None):
  # print "evaluate resid in dict", residue.resname in data_dict, residue.resname
  if residue.resname in data_dict:
    ...
    axis = None
    while ctr < len(axes):
      if residue_atoms[axes[ctr][0]].name.strip().upper() == data_dict[residue.resname][1].strip().upper() and \
          residue_atoms[axes[ctr][1]].name.strip().upper() == data_dict[residue.resname][2].strip().upper():
        axis = axes[ctr]
    ...
    # print "axis", axis
    # print "atoms tr", atoms_to_rotate
    # actually rotating atoms

    for atom in atoms_to_rotate:
      # print "rotating", residue.atoms()[atom].id_str(), "around",
      # print residue.atoms()[axis[0]].id_str(), residue.atoms()[axis[1]].id_str()
      ...
      # print "old/new xyz", residue.atoms()[atom].xyz, new_xyz
...
def should_be_flipped(residue_1, residue_2):
  ...
  if residue_1.resname in data_dict:

    tor_1_sites = []
    for aname in data_dict[residue_1.resname][:4]:
    ...
    tor_23_sites = []
    for aname in data_dict[residue_1.resname][:5]:
    ...
    tor3 = dihedral_angle(
      ...
      deg=True)
    # print "tors in should be flipped:", tor1, tor2, tor3
