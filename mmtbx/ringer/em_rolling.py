...
class RingerDict(object):
  ...
  def add_residue(self, residue):
    if residue.chain_id not in list(self.dict.keys()):
      self.dict[residue.chain_id] = {}
    if 1 in list(residue._angles.keys()):
      self.dict[residue.chain_id][residue.resid] = residue._angles[1]

  def get_peak(self, chain_id, residue_id):
    if (chain_id in list(self.dict.keys()) and residue_id in list(self.dict[chain_id].keys())):
...
def identify_regions(results,
      ...
      out=sys.stdout):
  import numpy as np
  for chain, chain_out in results.items():
    outliers = []
...
def plot_results (results_a, dir_name, threshold, graph=False, save=True) :
  ...
  for chain in list(results_a.keys()) :
    fig = plt.figure(1)
    _plot_results_for_chain(
      figure=fig,
      results_a=results_a,
