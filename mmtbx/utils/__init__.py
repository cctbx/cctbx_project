...
class xray_structures_from_processed_pdb_file(object):
  ...
  def __init__(self, processed_pdb_file, scattering_table, d_min, log = None):
    ...
    if(len(model_indices)>1):
       ...
       for ran in ranges:
         sel = flex.size_t(list(range(ran[0],ran[1])))
...
def sample_and_fix_rotamer(
  ...
  for backrub_angle in sample_backrub_angles:
    if verbose:
      print("  Backrub angle:", backrub_angle, file=log)
  ...
  if len(all_inf) == 0:
    return
  if verbose:
    print("Setting best available rotamer:", all_inf[-1][:-1], file=log)
    ...
    for chain in model.chains():
      for conf in chain.conformers():
        ...
        for i_res, res in enumerate(residues):
          fix_residue = False
          if verbose:
            print("Working on", res.id_str(), end=' ', file=log)
          ...
          if non_outliers_to_check is not None:
            if non_outliers_to_check[res.atoms()[0].i_seq]:
              fix_residue = True
              if verbose:
                print("Checking", file=log)
          ev = rotamer_manager.evaluate_residue_2(res)
          if ev == "OUTLIER":
            fix_residue = True
            if verbose:
              print("OUTLIER", file=log)
          if not fix_residue:
            if verbose:
              print("Skipping", file=log)
          if fix_residue:
