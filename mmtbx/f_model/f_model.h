...
class core
{
  public:
    ...
    core(af::shared<FloatType>                  const& f_obs_,
         af::shared<ComplexType>                const& f_calc_,
    ...
    :
      f_obs(f_obs_), f_calc(f_calc_), f_mask(f_mask_),   k_sol(k_sol_),
      ...
      a(uc_.fractionalization_matrix())
    {
      ...
      MMTBX_ASSERT(f_obs.size() == hkl.size()   );
      MMTBX_ASSERT(f_obs.size() == f_calc.size());
      MMTBX_ASSERT(f_obs.size() == f_calc.size());
