...
# XXX refactored version of twin_analyses.py - this will eventually be a
# complete replacement but it will require synchronized changes to the rest of
# Xtriage and the Phenix GUI.  Given the size of this module and the amount of
# changes involved it made more sense to check in intermediate steps.
...
from libtbx.str_utils import format_value, make_header, make_sub_header
...
PM: Pseudomerohedral twin law""")
  ...
  def show(self, out=None):
    ...
    if (len(self.operators) != 0) :
      make_sub_header("Possible twin laws", out=out)
      print >> out, self.table.format()
      print >> out, """
...
""" % ({ "m" : self.m, "pm" : self.pm, "n_op" : len(self.operators) })
      ...
      if self.coset_decomposition :
        make_sub_header("Details of automated twin law derivation", out=out)
        print >> out, """
...
"""
      ...
      print >> out, ""
      print >> out, "No twin laws are possible for this crystal lattice."
      print >> out, ""
...
class detect_pseudo_translations(object):
  ...
  def show(self,out=None):
    if out is None:
      out = sys.stdout
    make_sub_header("Patterson analyses", out=out)
...
class wilson_moments(object):
  ...
  def show(self,out=None):
    if out is None:
      out = sys.stdout
    make_sub_header("Wilson ratio and moments", out=out)
...
class n_z_test(object):
  ...
  def show(self,out=None):
    if out is None:
      out = sys.stdout
    make_sub_header("NZ test", out=out)
    print >> out, """
...
class l_test(object):
  ...
  def show(self,out=None):
    if out is None:
      out=sys.stdout
    make_sub_header("L test for acentric data", out=out)
    print >> out, """
...
this is the case. Carefull reprocessing and (twin)refinement for all cases
...
"""
  ...
  def show(self,out=None):
    if out is None:
      out = sys.stdout
    make_header("Twinning and intensity statistics summary", out=out)
    make_sub_header("Statistics independent of twin laws", out=out)
    print >> out, """\
...
"""
    if len(self.twin_laws)>0:
      make_sub_header("Statistics depending on twin laws", out=out)
    ...
    if self.patterson_height is not None:
      ...
      print >> out
    print >> out
    make_sub_header("Final verdict", out=out)
...
class symmetry_issues(object):
  ...
  def show(self, out=None):
    ...
    print >> out
    print >> out
    make_header("Exploring higher metric symmetry", out=out)
...
class r_values(object):
  ...
  def show (self, out=sys.stdout):
    make_sub_header("R vs R statistics", out=out)
    print >> out, """\
  R_abs_twin = <|I1-I2|>/<|I1+I2|>
    (Lebedev, Vagin, Murshudov. Acta Cryst. (2006). D62, 83-95)
  R_abs_twin observed data   : %4.3f""" % (self.r_abs_obs)
    if self.r_abs_calc is not None:
      print >> out , "   R_abs_twin calculated data : %4.3f" % self.r_abs_calc
    print >> out, """
  R_sq_twin = <(I1-I2)^2>/<(I1+I2)^2>"
  R_sq_twin observed data    : %4.3f""" % self.r_sq_obs
    if self.r_abs_calc is not None :
      print >> out , "   R_sq_twin calculated data  : %4.3f" % self.r_sq_calc
      print >> out
      """
      ...
      print >> out , "  No calculated data available."
      print >> out , "  R_twin for calculated data not determined."
    print >> out
...
class twin_analyses(object):
  ...
  def __init__(self,
    ...
    self.twin_summary = twin_results_interpretation(
      ...
      l_test=self.l_test,
      translational_pseudo_symmetry=self.translation_pseudo_symmetry,
...
def twin_analyses_brief(miller_array,
  ...
  try:
    summary = miller_array.analyze_intensity_statistics(cut_off=cut_off,
      log=out)
  except Exception, e :
    print "Twin analysis failed:", str(e)
    return None
  else :
