...
Options:
  ...
  ignore_missing_restraints=False  Continue if some ligands lack restraints
...
""" % locals()
  ...
  ignore_missing_restraints = False
    .type = bool
    .help = '''Continue processing when restraints are missing for some \
               residues (e.g. unknown ligands). Those residues will be \
               excluded from hydrogen addition but their heavy atoms will \
               still participate in contact analysis. A warning will list \
               the excluded residues.'''
