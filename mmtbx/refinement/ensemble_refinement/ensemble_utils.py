...
#      x.shift_us(b_shift = shift_b_iso)
  ...
  def ensemble_mean_geometry_stats(self,
    ...
    for proxy, info in ensemble_bond_deltas.items():
    ...
    for proxy, info in ensemble_angle_deltas.items():
      assert info[1] == ensemble_size
      mean_delta = info[0] / info[1]
    ...
    for proxy, info in ensemble_chirality_deltas.items():
      assert info[1] == ensemble_size
      mean_delta = info[0] / info[1]
    ...
    for proxy, info in ensemble_planarity_deltas.items():
      assert info[1] == ensemble_size
      mean_delta = info[0] / info[1]
    ...
    for proxy, info in ensemble_dihedral_deltas.items():
      assert info[1] == ensemble_size
      mean_delta = info[0] / info[1]
