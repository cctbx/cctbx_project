...
class run(object):
  def __init__(
    ...
    # Explode and refine: per residue scorer
    sc = scorer_per_residue(
    ...
    self.show_target(prefix="flexible targets:")
    # Explode and refine: per chain scorer
    self.pdb_hierarchy.adopt_xray_structure(self.ensemble_xrs[0])
    sc = scorer_whole_chain(
      pdb_hierarchy = self.pdb_hierarchy,
      unit_cell     = self.xray_structure.unit_cell(),
      map_data      = map_data)
    self.explode_and_refine(scorer=sc, weight=weights[1])#flex.mean(weights))
    self.show_target(prefix="explode-refine:")
    #
    self.run_refine_flexible_rmsd_targets(weights = weights)
    self.show_target(prefix="flexible targets:")
  ...
  def simulated_annealing(self, weight):
    ...
    #print dir(params)
    params.start_temperature=100000
    params.final_temperature=0
    #params.time_step=0.0005 # 0.0005
    params.cool_rate = 50000
    ...
    for it in xrange(20):
      ...
      states.add(sites_cart = tmp.sites_cart())
      result.append(tmp.deep_copy_scatterers())
    print "SA: %6.4f"%(time.time()-t0)
    states.write(file_name="SA.pdb")
  ...
  def run_refine_flexible_rmsd_targets(self, weights=None):
    ...
    #for macro_cycle in range(1, self.number_of_macro_cycles+1):
    #  scale = float(self.number_of_macro_cycles)/macro_cycle
    #  print self.target_bond_rmsd*scale, self.target_angle_rmsd*scale
    for i, pair in enumerate(pairs):
      ...
      ro = mmtbx.refinement.real_space.individual_sites.easy(
        ...
        rms_bonds_limit             = pair[0],#self.target_bond_rmsd*scale,
        rms_angles_limit            = pair[1],#self.target_angle_rmsd*scale,
        max_iterations              = 50,
        selection                   = None,
        w                           = w,
  ...
  def explode_and_refine(self, scorer, weight):
    #for trial in xrange(self.number_of_trials):
    ...
    states.write(file_name="EAR.pdb")
